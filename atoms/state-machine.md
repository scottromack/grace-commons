...
State Machine records a single named workflow — think of it as a token moving through a flowchart — where the flowchart (the set of valid states and the transitions between them) is declared by the system deploying it rather than fixed by this atom.

When the workflow is created, the deployer provides the complete map: every state, every allowed move between states (each move is a named action, optionally gated by a condition the calling system must assert is satisfied), the starting state, and any states that are permanent endpoints. From that moment forward the map is frozen — it cannot be changed for this workflow.

The atom then enforces three guarantees for the lifetime of the workflow: only declared moves are allowed (an undeclared move is refused); exactly one current state exists at all times; and every move that succeeds is recorded permanently, in order, with enough context to replay the full history and arrive at the current state.

These guarantees make the system useful for regulated processes (a pharmaceutical batch that must prove it moved only through the approved sequence of quality-control states) and for any process where you need an unambiguous, auditable record of what happened and in what order. This atom is the general pattern; [Approval Step](./approval-step.md) is one specific kind of state machine with its states and rules fixed in advance, whose workflows do not need their own declaration.
...
Identity 13: The atom MUST NOT identify a workflow by the subject reference.
Identity 14: The atom MUST admit a second workflow carrying a recorded subject reference.
Identity 15: The atom MUST NOT confirm that a subject reference names a known subject.
...
State 2: A workflow MAY carry subject reference.
State 3: A workflow MAY carry instance metadata.
State 4: EVERY history entry MUST carry transition id, sequence number, from state, to state, action and firing instant.
State 5: A history entry MAY carry actor reference.
...
Operation 2: IF a supplied actor reference EQUALS blank THEN an action MUST answer invalid-request.
Operation 3: IF now PRECEDES the resolved instantiation instant THEN [Instantiate] MUST answer invalid-request.
...
Operation 19: IF now PRECEDES the resolved firing instant THEN [Fire] MUST answer invalid-request.
Operation 20: IF the resolved firing instant PRECEDES the workflow's instantiation instant THEN [Fire] MUST answer invalid-request.
...
Operation 55: An ordering rule MUST NOT rest on firing instant.
```
...
Term firing instant: the instant a declared transition fired — a [Firing Instant]; a best-effort annotation, and never an order source.

Term instantiation instant: the instant a workflow was created — an [Instantiation Instant].

Term resolved firing instant: the firing instant the history entry carries — the supplied value where one exists, and now otherwise.

Term resolved instantiation instant: the instantiation instant the workflow carries — the supplied value where one exists, and now otherwise.

Term actor reference: the opaque reference naming who fired a declared transition — an [Actor Reference]; optional, and recorded where supplied.
...
Term filter axes: transition id | sequence number | from state | to state | action | actor reference | firing instant — the seven axes [History] accepts, and no others.

Term admitted instantiate: an [Instantiate] call whose declaration, actor reference and resolved instantiation instant the guards all admit.

Term admitted fire: a [Fire] call whose instance id names a live workflow, whose action matches a declared transition the current state offers, whose guard the caller asserted where one is carried, and whose actor reference and resolved firing instant the guards admit.
...
| 6 | the gate clears, actor reference is blank or the resolved firing instant is out of bounds | invalid-request |
...
Row 6 is the one that surprises, and it is deliberate: attribution and temporal checks run *after* the gate. The alternative — checking firing instant before the guard — would tell a caller their timestamp is wrong on a transition they were never allowed to make, which leaks the declaration's shape to a caller the declaration refuses.
...
Operation 55 is the discipline [Event Log](./event-log.md) set and this atom inherits: firing instant is best-effort and sequence number is the order. Under a skewing clock a later history entry may legitimately carry an earlier firing instant, and no invariant here is at risk from it.
...
- **Invariant 2 — Exactly one current state.**
  ```
  Invariant 2.1: EVERY workflow MUST stand in EXACTLY ONE member of the workflow's states.
  ```
...
- **Invariant 5 — History append-only and complete.**
  ...
  Invariant 5.2: A workflow's history entry count MUST equal the workflow's admitted fire count.
  ```
...
- **Invariant 9 — Transition attribution completeness.**
  ```
  Invariant 9.1: A recorded actor reference MUST stand non-blank.
  ```
  WHY: the entry is complete for forensic replay whether or not actor reference was supplied. Attribution is deployment policy here, not an atom-level mandate — which is exactly the gap [Actor Identity](./actor-identity.md) closes where a regulator needs the actor bound rather than named.
- **Invariant 10 — Store durability.**
  ```
  Invariant 10.1: The atom MUST NOT remove a workflow from the store.
  Invariant 10.2: The atom MUST NOT remove a history entry from a workflow.
...
Declaration 4: EVERY enforcement decision MUST rest on the workflow's declaration.
...
Term declaration: the immutable map governing one workflow — a [Declaration]; carries states, the declared transitions, the initial state and the terminal states.

Term states: the named states one declaration admits — a [States]; every state name the workflow may stand in.

Term initial state: the state a workflow stands in at [Instantiate] — an [Initial State]; a member of states and never a terminal state.
...
`fire("wf_01HQ…", "release", guard_satisfied: true, actor_ref: "qa-lead-m.ross")` → `released`. The workflow now stands in a terminal state.
...
`fire("wf_01HQ…", "test")` against the released workflow → terminal. Not invalid-transition, even though no `test` edge leaves `released` — an absorbed workflow refuses every action, and saying so sends the caller to the right problem (Operation 13, Operation 16).

`fire("wf_01HQ…", "expedite")` against a live workflow in `tested` → invalid-transition. No declared transition matches, and there is no wildcard (Operation 15, Operation 32).
...
`instantiate(declaration)` where `initial state: released` and `released` is terminal → invalid-declaration. A workflow born absorbed accepts nothing (Declaration 9).

`instantiate(declaration)` carrying an edge out of `released` → invalid-declaration (Declaration 12).

`fire("wf_01HQ…", "test", fired_at: "2020-01-01")` against a workflow instantiated in 2026 → invalid-request. A transition cannot fire before the workflow existed (Operation 20).
...
Check 8.1: An auditor MUST find transition id, sequence number, from state, to state, action and firing instant on EVERY history entry (State 4).
Check 8.2: An auditor MUST find EVERY recorded actor reference non-blank (Invariant 9.1).
```
...
External check 3: A deployment needing an actor reference bound to an actor MUST read the composing Actor Identity attestation (Non-goal 11).
```

WHY:
External check 2 is the check that could not stay in the conformance list. Counting a workflow's history entries is trivial from the store; matching that count against the number of *admitted* fires needs the fire answers captured at call time, and a production auditor reading the store cannot enumerate them — the store is exactly what would be missing an entry. Check 5.1 is the store-alone substitute: a gap-free run from one to the entry count catches a lost entry from the other direction.
...
Non-goal 3: The atom MUST NOT stand a workflow in two states at once.
...
Non-goal 11: The atom MUST NOT bind an actor reference to an actor.
...
Non-goal 15: The atom MUST NOT bound a workflow's retention.
...
WHY:
Non-goal 3 through 5 bound the atom to a single active state, and the bound is what makes every other guarantee statable. Parallel workflows — a workflow live in several states, a join firing when all branches complete — have no single current state to be exactly one of, so Invariant 2, Invariant 7 and the whole replay story would need rewriting rather than extending. This atom is the single-active-state primitive and a fork-join pattern is a sibling, not a setting.

Non-goal 8 and Non-goal 9 together answer the question every deployer asks second: *can I change the flowchart?* Not for a live workflow, ever. The declaration is a value the workflow holds, so two workflows created from one template are two independent copies and editing the template moves neither. Where a deployer needs a canonical template that new workflows pick up, that is a definition registry holding the template and a calling system supplying it at [Instantiate] — this atom receives a declaration and never asks where it came from.

Non-goal 19 is the limit an auditor most often pushes against. *Show me every batch that never reached released* is not a question this atom answers: it knows what each workflow declared and what each workflow did, and it does not know which workflows ought to exist or what a batch of a given type is expected to terminate in. The composing system drives that analysis with this atom's records as input.
...
```
Clock semantics 4: A firing instant MUST NOT bound a later history entry's firing instant.
...
WHY:
Clock semantics 4 is the rule that looks like a gap and is a commitment. Wall-time monotonicity across history entries is deliberately *not* enforced: under a skewing or resynchronized clock a later transition can legitimately carry an earlier firing instant, and an atom that refused it would reject correct history to protect an annotation. sequence number is the order (Invariant 6.3, Operation 55), so nothing is lost. A backdated firing instant is accepted within the workflow's own lifetime — documenting a transition recognized late is valid — and the only temporal bounds are the two that are structural: not after now, and not before the workflow existed (Operation 19, Operation 20).

### Concurrency

```
Concurrency 1: The implementation MUST serialize two [Fire] calls against one workflow.
Concurrency 2: A serialized [Fire] MUST read the current state the prior [Fire] left.
```

WHY:
Unlike [Selective Disclosure](./selective-disclosure.md), whose concurrent records contend over nothing, two fires against one workflow contend over the current state itself — the second call's matched transition depends on where the first left the workflow. So the second may succeed, may answer invalid-transition, or may answer terminal, and which of the three is a fact about the declaration rather than a race. Serialization is what makes the outcome a fact at all.
...
Term string input: instance id, action, actor reference, subject reference, a state name, a guard OR a filter's value — every caller-supplied string this atom accepts.
...
Composition note 5: A composing pattern MUST own the attestation binding an actor reference.
...
[Approval Step](./approval-step.md) is the fixed-state sibling and the clearest way to see what this atom trades away: its states are known to an evaluator who has read only the spec, and this atom's are known only to one who has read the workflow. [Actor Identity](./actor-identity.md) makes actor reference survive an authorship challenge under 21 CFR Part 11 and SOX (Sarbanes-Oxley Act) section 404; [Tamper Evidence](./tamper-evidence.md) seals the history for court admissibility; [Retention Window](./retention-window.md) bounds how long workflows are kept under GDPR (General Data Protection Regulation) Article 17, HIPAA (Health Insurance Portability and Accountability Act) section 164.530(j) and FRCP (Federal Rules of Civil Procedure) Rule 37(e); [Audit Trail](../compositions/audit-trail.md) is the regulated-evidence layer each admitted fire lands in.
...
Term records: workflow — one running workflow, carrying instance id, a declaration, a current state, a transition history, next sequence number and, where supplied, subject reference and instance metadata. history entry — one recorded transition, carrying transition id, sequence number, from state, to state, action, firing instant and, where supplied, actor reference and guard satisfied.
...
Term terms: workflow, instance id, history entry, transition id, subject reference, instance metadata, store instance, seam, transition, declared transition, declaration, states, initial state, terminal states, well-formed declaration, now, business caller, addressed action, writing action, reading action, current state, next sequence number, sequence number, firing instant, instantiation instant, resolved firing instant, resolved instantiation instant, actor reference, guard, guard satisfied, matched transition, filter axes, admitted instantiate, admitted fire, admitted history, string input, blank, uncommitted crash, dangling transition.

#### Instantiate

The behavior that validates a caller-supplied [Declaration] and creates a new workflow in its [Initial State], assigning a fresh [Workflow Id], recording [Workflow Metadata], [Actor Reference], and [Instantiation Instant], and starting an empty [Transition History]. Rejected [Invalid Declaration] for any declaration defect, [Invalid Request] for a bad [Actor Reference] or [Instantiation Instant], or [Storage Failure].

Kind: Operation

#### Fire

The behavior that advances a workflow along a declared transition: it looks up the unique transition matching the current state and the supplied [Action], gates on [Guard] / [Guard Satisfied], appends one immutable history entry, and sets [Current State] to the transition's [To State]. Rejected [Not Known], [Terminal], [Invalid Transition], [Guard Not Satisfied], [Invalid Request], or [Storage Failure].

Kind: Operation

#### Current

The read-only query returning a workflow's [Current State] name. Rejected [Not Known] or [Invalid Request].

Kind: Operation

#### History

The read-only query returning a workflow's [Transition History] in [Sequence Number] ascending order, filterable by [Transition Id], [Sequence Number], [From State], [To State], [Action], [Actor Reference], or a [Firing Instant] range. Rejected [Not Known], [Invalid Query], or [Invalid Request].

Kind: Operation

#### Read Declaration

The read-only query returning the workflow's immutable [Declaration] exactly as supplied at [Instantiate]. Rejected [Not Known] or [Invalid Request].

Kind: Operation

#### Workflow Id

The opaque, immutable, system-generated identity of a workflow — assigned on [Instantiate], never reused, byte-order sortable for deterministic [History] ordering. It is the workflow's identity; the [Declaration], [Current State], and [Transition History] are properties.
...
The workflow's current state — a member of the declared [States] (Invariant 2). Set to [Initial State] on [Instantiate]; updated to the fired transition's [To State] on each [Fire]. Always replay-derivable from the [Transition History] (Invariant 7).
...
The ordered, append-only sequence of transition history entries for a workflow (Invariant 5). Begins empty; each [Fire] appends exactly one entry; never shrinks; total-ordered by [Sequence Number] (Invariant 6).
...
The [Sequence Number] the next [Fire] will assign. Begins at 1; increments by one per successful [Fire]. Persistent workflow state that must survive restarts, or Invariant 6 breaks.
...
The immutable map supplied at [Instantiate] that governs a workflow — its [States], [Transitions], [Initial State], and [Terminal States]. Every enforcement decision derives from it; it never changes (Invariant 1) and is returned verbatim by [Read Declaration].
...
The wall-time the workflow was created, stamped at [Instantiate] (or caller-supplied; not in the future). The lower bound for every [Firing Instant] — a transition cannot predate instantiation.
...
The [Declaration]'s starting state — a member of [States], never a [Terminal States] member. It is the workflow's [Current State] at [Instantiate] and the replay origin (Invariant 7).
...
The state a declared transition (and the history entry it produces) departs from — a member of [States]. On a [Fire], it must equal the workflow's [Current State].
...
The state a declared transition (and the history entry it produces) arrives at — a member of [States]. On success it becomes the workflow's new [Current State] and the returned new_state.
...
The opaque, immutable, system-generated identity of a transition history entry — assigned by [Fire], never reused within the workflow.
...
The strictly increasing integer assigned to a history entry from [Next Sequence Number] at [Fire]. The clock-independent total-order source for the [Transition History] (Invariant 6); [Firing Instant] is not.
...
The best-effort wall-time a transition was recorded, stamped at [Fire] (or caller-supplied; not in the future; ≥ [Instantiation Instant]). Not required to be monotonic across entries — [Sequence Number] is the order source.
...
The rejection an action returns for a malformed argument — a null or whitespace-only [Workflow Id] or [Action], a bad [Actor Reference], or an [Instantiation Instant] / [Firing Instant] that is in the future or (for [Firing Instant]) before [Instantiation Instant].
...
The rejection [Fire], [Current], [History], or [Read Declaration] returns when the [Workflow Id] references no workflow in the store.
...
The rejection [Fire] returns when the workflow's [Current State] is a [Terminal States] member — terminal states are absorbing (Invariant 4).
...
The rejection [History] returns for a malformed filter — a null or whitespace-only [Transition Id], [From State], [To State], [Action], or [Actor Reference]; a reversed [Sequence Number] or [Firing Instant] range; or an unrecognized filter key.
...
[Instantiation Instant]: #instantiation-instant
[States]: #states
...
- **FDA 21 CFR Part 11 (Electronic Records; Electronic Signatures)** — for FDA-regulated contexts: each declared state transition constitutes a regulated electronic record; the transition history is the audit trail that Part 11 section 11.50 (attributability) and Part 11 section 11.70 (record linking to prevent removal, substitution, or falsification) require. Composition with [Actor Identity](./actor-identity.md) provides the section 11.50 attributability; composition with [Tamper Evidence](./tamper-evidence.md) provides the section 11.70 non-falsifiability. Invariants 3 (only-declared-transitions), 5 (history append-only and complete), and 7 (replay determinism) are the atom-level guarantees the regulated record depends on.
- **ISO 9001:2015 clause 8.5.1 (Control of production and service provision)** — requires that production and service provision activities be controlled by documented procedures, including controlled transitions at defined points in the process lifecycle. A State Machine workflow governing a production process is the documented process record section 8.5.1 anticipates; the declaration is the procedure map; the transition history is the per-workflow proof of execution.
- **BPMN 2.0 (Business Process Model and Notation 2.0 — an international standard for modeling business processes, published by the Object Management Group)** — this atom is the primitive behind a BPMN state diagram. A BPMN process model's states and sequence flows map directly to this atom's [States] and [Transitions]; a BPMN lane actor maps to an [Actor Reference] on [Fire] calls; a BPMN gateway condition maps to a [Guard] label on a declared transition.

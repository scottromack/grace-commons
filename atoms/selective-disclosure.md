...
```
Identity 1: The atom MUST identify a disclosure record by the disclosure id.
Identity 2: The host MUST allocate a disclosure id at the seam.
Identity 3: The transition MUST NOT allocate a disclosure id.
Identity 4: The atom MUST NOT change a disclosure id.
Identity 5: Two disclosure records in one store instance MUST NOT share a disclosure id.
Identity 6: The deployment MUST route EVERY call to one store instance.
Identity 7: The atom MUST NOT identify a disclosure record by the subject reference.
Identity 8: The atom MUST admit a second disclosure record carrying a recorded subject reference.
Identity 9: The atom MUST match a subject reference exactly.
Identity 10: The atom MUST match a recipient exactly.
Identity 11: The atom MUST NOT normalize a subject reference.
...
Identity 15: The deployment MUST choose a disclosure id format that sorts in lexicographic byte order.
```
...
Term store instance: one named disclosure store a call is routed to; disclosure id uniqueness ranges over one instance.

Term seam: the atom's I/O boundary as the section titled Logic Confinement Principle in `execution-contract.md` declares it; the host injects the clock reading and the disclosure id here.
...
subject reference is deliberately not an identity (Identity 7, Identity 8). One subject has many disclosures; each is its own accountability record, and collapsing them under a subject key would make the store answer *what is true of this subject* rather than *what happened to this subject's data*, which is the question the regulation asks.

### State

```
State 1: EVERY disclosure record MUST carry disclosure id, subject reference, recipient, scope, authority type, authority reference and disclosure instant.
...
WHY:
Clock skew between a caller and the seam can push a disclosure instant the caller believes is current past the injected reading, and Operation 7 rejects it. That is the correct rejection: the temporal invariant is enforced against the injected now, not against the caller's belief about the time.
...
```
Operation 1: IF subject reference EQUALS blank THEN [Record] MUST answer invalid-request.
...
Operation 7: IF now PRECEDES the resolved disclosure instant THEN [Record] MUST answer invalid-request.
...
Operation 12: An admitted record MUST stamp the disclosure record's disclosure instant from the resolved disclosure instant.
Operation 13: An admitted record MUST answer the disclosure id.
...
Operation 18: An admitted read MUST answer the matching disclosure records in disclosure instant ascending order.
Operation 19: An admitted read MUST order two disclosure records sharing a disclosure instant by disclosure id ascending.
...
Operation 23: IF a disclosure id, subject reference OR recipient filter's value EQUALS blank THEN [Read] MUST answer invalid-query.
Operation 24: IF an authority type filter's value IS NOT IN the authority types THEN [Read] MUST answer invalid-query.
Operation 25: IF a disclosure instant range's before PRECEDES the range's after THEN [Read] MUST answer invalid-query.
Operation 26: An admitted read MUST match a disclosure instant range as a closed interval.
...
Deleted: Operation 34. Execution Contract Logic confinement 3 owns it.
```
...
Term disclosure instant: the instant the disclosure happened — a [Disclosure Instant]; caller-supplied or resolved to now, and never later than now.

Term resolved disclosure instant: the disclosure instant the record carries — the supplied value where one exists, and now otherwise.

Term field-level precondition: Operation 1 through 7 — every check [Record] makes before the authority type is read.

Term filter axes: disclosure id | subject reference | recipient | authority type | disclosure instant — the five axes [Read] accepts, and no others.

Term admitted record: a [Record] call whose fields, resolved disclosure instant and authority type the guards all admit.

Term admitted read: a [Read] call whose every filter axis and filter value the guards admit.

| # | Condition | [Record] answers |
|---|---|---|
| 1 | a field is blank, or the resolved disclosure instant follows now | invalid-request |
...
WHY:
Operation 8 is the precedence rule, and it is load-bearing rather than tidy. Without it a call carrying both a blank subject reference and a misspelled authority type could answer either way, and a caller repairing the authority type would then discover the blank field on the next attempt — two round trips to learn what one answer already knew. The same shape governs Operation 15: storage-failure means *the call was good and the store was not*, so landing it while a precondition is unchecked would tell the caller to retry something that will never succeed.

Operation 7 is the atom's one genuine execution-time validation — the residual the logic-confinement discipline marks rather than derives away. A disclosure cannot be recorded as having happened later than the moment of recording, and that determination can only be made against the injected reading. The guard is a pure function of the resolved disclosure instant and now, and it writes nothing when it fails.
...
- **Invariant 3 — Field completeness.**
  ```
  Invariant 3.1: EVERY disclosure record MUST carry a disclosure instant.
  Invariant 3.2: EVERY disclosure record's subject reference, recipient, scope and authority reference MUST stand non-blank.
  ```
- **Invariant 4 — Temporal soundness.**
  ```
  Invariant 4.1: EVERY disclosure record's disclosure instant MUST NOT follow the now the record's creation read.
  ```
  WHY: a record whose disclosure instant is later than the instant it was written claims the system recorded a disclosure that had not happened yet. The constraint is enforced against the resolved value — caller-supplied or defaulted — by Operation 7's guard, before the write.
...
- **Invariant 6 — Store durability and append-only nature.**
  ...
  Invariant 6.3: An answered disclosure id MUST name a durably persisted disclosure record.
  ```
...
A data subject exercises their access right. `read({subject_ref: "patient-88213"})` answers three records in disclosure instant ascending order — the regulatory report first at 09:15, then the consent disclosure at 14:02, then the compelled one — each carrying its recipient, scope, authority type and authority reference. `read({subject_ref: "patient-88213", authority_type: "consent"})` narrows to one. The regulatory question is answered from the store alone.

### Rejection paths

`record("", "Northgate Research Institute", "medical-record:summary", {type: consent, reference: "consent-3301"})` → invalid-request. An empty subject reference EQUALS blank (Operation 1, String 5).
...
```
Check 2.1: An auditor MUST find disclosure id, subject reference, recipient, scope, authority type, authority reference and disclosure instant on EVERY disclosure record (State 1, Invariant 3.1).
Check 2.2: An auditor MUST find EVERY disclosure record's subject reference, recipient, scope and authority reference non-blank (Invariant 3.2).
...
Check 6.1: An auditor MUST find [Record] answering invalid-request for a disclosure instant following now (Operation 7).
Check 6.2: An auditor MUST find no disclosure record's disclosure instant following the record's creation instant (Invariant 4.1).
```
...
Non-goal 21: The atom MUST NOT bound disclosure instant from below.
...
Non-goal 21 is deliberate asymmetry. disclosure instant is bounded above because a future disclosure has not happened, and unbounded below because a disclosure recognized long after the fact — a breach discovered in an audit, a transmission found in a log review — is exactly the case the accounting must be able to capture.
...
```
Concurrency 1: The atom MUST NOT serialize two [Record] calls against one subject reference.
Concurrency 2: A [Record] call's answer MUST NOT rest on a concurrent [Record] call's answer.
Concurrency 3: The implementation MUST issue EXACTLY ONE disclosure id per admitted record.
```
...
Term string input: subject reference, recipient, scope, authority reference OR a filter's value — every caller-supplied string this atom accepts.
...
Term records: disclosure record — one recorded disclosure, carrying disclosure id, subject reference, recipient, scope, authority type, authority reference and disclosure instant.
...
Term terms: disclosure record, disclosure id, subject reference, recipient, scope, store instance, seam, transition, now, business caller, authority, authority type, authority types, authority reference, disclosure instant, resolved disclosure instant, field-level precondition, filter axes, admitted record, string input, blank.

#### Record

The behavior that appends one immutable disclosure record: it takes the injected `id_t` as the record's [Disclosure Id], stores [Subject Reference], [Recipient], [Scope], [Authority] (both [Authority Type] and [Authority Reference]), and [Disclosure Instant] (the injected clock now if not supplied; rejected [Invalid Request] if in the future), and returns `recorded(disclosure_id)`. Rejection priority: [Invalid Request] → [Unknown Authority Type] → [Storage Failure].

Kind: Operation

#### Read

The read-only query returning the matching disclosure records in [Disclosure Instant] then [Disclosure Id] ascending order, filtered by any of [Disclosure Id], [Subject Reference], [Recipient], [Authority Type], or a [Disclosure Instant] range. Never writes; an empty result is not a rejection. Rejected [Invalid Query] for a malformed filter.
...
The rejection [Record] returns when a required field ([Subject Reference], [Recipient], [Scope], or [Authority Reference]) is missing or whitespace-only, [Authority] is structurally malformed, or the resolved [Disclosure Instant] is in the future.
...
The rejection [Read] returns when a filter value is malformed — a null or whitespace [Disclosure Id], [Subject Reference], or [Recipient]; an [Authority Type] outside the three values; a reversed [Disclosure Instant] range; or an unrecognized filter key.
...
[Disclosure Instant]: #disclosure-instant
...
- **HIPAA section 164.528 (Accounting of disclosures of protected health information)** — individuals have the right to receive an accounting of disclosures of their PHI (Protected Health Information — individually identifiable health data covered by HIPAA) made by a covered entity or business associate. The accounting must include the date of each disclosure, the name and address of the recipient, a brief description of the PHI disclosed, and a brief statement of the purpose. Each field maps directly: [Disclosure Instant] (date), [Recipient] (recipient name), [Scope] (description of PHI), [Authority Reference] (purpose statement). The atom is the structural implementation of section 164.528's accounting obligation.

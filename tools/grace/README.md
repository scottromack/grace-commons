...
| **F-prefix-first** | A tombstone opening a fenced block: `Surface 18` classifies the block by its first line, so a `NOTE: … deleted` written at the top silently demotes every rule beneath it. The edit looks like housekeeping and reads like a deletion — it happened once, during CR-15's repair pass, and the checker caught it. |
| **V-dup-vocab** | A `Terms › record verbs` or `Terms › terms` line naming the same name twice. The corpus's rewrite template shipped `identify` twice and five atoms inherited it, plus two compositions with their own — the defect a template repeats is the defect no reader sees (CR-16). |

...
| **D-wire-spelling** | A term entry's English name inside anything that is wire: a signature, which opens `name(input, …)`; a record or event shape, which opens `{`; or a braced shape inline on any line, `{finding, kind, act_key, first_seen}` on a declaration, which is the same surface in a different position (council read 140). The wire is written where the wire belongs, so such a block carries the `Projection:` spelling. The test is deliberately narrow — the Ledger and the rule blocks are fenced too, and name term entries by their headings, where English is right. Task 25's sweep uses the opposite test, keeping any fenced block that is not rule text, because a sweep that guesses wrong corrupts a wire surface where a gate that guesses wrong only cries wolf. Found one site corpus-wide, in a signature that had answered English since `A signature has one owner` (council read 138). |
| **D-code-token** | A code span that is only lower-case words and underscores — an identifier with a space in it, which is what a rename leaves when it reaches a wire spelling it should have kept: `compensable workflow_id`. The test is narrow on purpose, because Term code span admits an expression, a form and a citation, all of which carry spaces. Five fixtures; found 31 sites in two specifications on its first run, one of them migrated (council read 164). |
| **D-condition-form**, operand type | One operator, one operand type (Earned vocabulary 18, GRACE-lang v0.61): EXCEEDS beside an instant fires — instants compare with `PRECEDES`, negated `DOES NOT PRECEDE` — and a lower-case *precedes* fires, because the word is an operator rather than prose. A duration keeps EXCEEDS. Five fixtures. |
| **D-condition-form** | A condition outside the operators (Earned vocabulary 6 through 12, GRACE-lang v0.52): `NOT EXISTS`, which is `no thing EXISTS` for a thing and `EQUALS blank` for a value; `is blank`, which is `EQUALS blank`; `NOT EXCEEDS`, which is DOES NOT EXCEED (Earned vocabulary 17); a state tested with *stands in* or *stands outside*, which is `the record's state EQUALS member` (Earned vocabulary 15); `EXISTS in`, which is `IS IN`; `!=`, which is DOES NOT EQUAL; `=` in a rule, which is EQUALS in a test and `field set to value` in a write; and a signature input written as the subject of EXISTS. A declaration may keep the value-set form's `=`. |
| **W-ge-disjunction** | Gating since GRACE-lang v0.57, the name kept: *at least* spelled as `x EXCEEDS y OR x EQUALS y` over one operand pair, where `y DOES NOT EXCEED x` says it in one arm (Earned vocabulary 17). |

...
| **R-caps** / **W-caps** *(advisory)* | A word in capitals that is not a reserved token (Casing 2, Casing 6). Gating when the word is grammar-shaped — a watched or provisional form, or an inflection of a reserved token (`WHILE`, `DEGRADES TO`, `EXIST`); advisory when it is a proper noun in capitals (`SOX`, `GRACE`), which awaits a ruling. Both sets derive from `GRACE-lang.md`. Landed at council read 79, when three such words were found in rules both checkers passed. |

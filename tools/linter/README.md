...
| **G-status-grammar** | A pattern with no `## Status` section, or whose status line does not start with one backticked token conforming to the pinned grammar (`pressure-testing.md` section Status line format, pinned 2026-06-11). |
...
| **X-section-title** | A citation `the section titled X in \u0060file\u0060` — or `the row titled X` — in an atom or a composition whose title is no heading, bold-bullet heading or docket row of the file it names, or of its own page where it names none. A section carries no label, so nothing else resolves it (council read 126). |
| **X-contract-label** | A citation `Execution Contract <Family> <N>` in an atom or a composition that names no label `execution-contract.md` carries (`**Logic confinement 3:**`, `**Conformance 2:**`). The contract's rules are labelled prose, so no rule block resolves them (council read 124). |
...
| **F-composes-list** | A composition whose `## Composes` section lists no linked constituent, `- **[Name](path)** — role.` (`spec-format.md` section Composes). The generated catalogue, graph and pattern data read that list and nothing else. |
...
| **F-renumber** | An invariant that changed its number or lost it, read from git: against the spec at HEAD when the working tree differs, and against the spec before the commit that migrated it. A number declared there is still declared or tombstoned; a number tombstoned there is not declared again; an invariant's title does not reappear under another number (its content words overlap the other title by half or more, and by more than the title now at its own number). Hard invariant 26 and Hard invariant 27. |
| **H-heading** | A migrated spec's `##` / `###` headings against the section titled *Heading standard* in `spec-format.md`, which the check reads rather than copies: an unknown section, a known heading at the wrong level or under the wrong parent, headings out of table order (unplaced ones after the placed, alphabetically), a required heading missing, and a second name for a row — a name the section titled *Retired heading names* in `spec-format.md` lists, or a row's name with a plural. |
| **H-classification** | `execution-contract.md` section Section-name classification in both directions: every heading the standard names is classified, and every classified name is still named by `spec-format.md`. The check that section specified in June. |

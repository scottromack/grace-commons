...
*Correction 2026-09-10.* **This split measured English words, not forms, and stands as the record of that error.** It ignored case, and it matched fragments and synonyms: `IS DERIVED FROM` was counted as `derived from` (48 files; the form in any case is in 7), `MUST NOT` with *never*, `EXACTLY ONE OF` with *at most one*. `ONLY IF` is in 29 files in any case, below the stated range, and the second list's numbers are occurrences under a heading that says files. Counted as whole-word uppercase forms over the 56 files of `atoms/` and `compositions/`, `MUST` is in 3 files, `MUST NOT` in 1, and every other form named here in none. The re-measurement is in [`roadmap.md`](./roadmap.md) debt #21 and the rule it produced in [`pressure-testing.md`](./pressure-testing.md) §*Measure the form, not the word*.

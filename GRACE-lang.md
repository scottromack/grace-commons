# GRACE lang v0.52 — Minimal Earned Grammar
...
Term condition operator: EQUALS | DOES NOT EQUAL | EXISTS | IS IN | IS NOT IN | EXCEEDS | AND (flat) | OR (flat, inclusive).

Term blank: a value that is absent, empty, or carries only whitespace — a caller's argument, a stored field or a deployment setting that holds nothing.

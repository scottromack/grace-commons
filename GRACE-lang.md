# GRACE lang v0.40 — Minimal Earned Grammar
...
NOTE:
The promotion census is `tools/grace/cites.py --drift`'s and `lint.py`'s `W-stale-census` compares it against the corpus on every run, so a candidate's count is never hand-written (council read 33, council read 52). Eleven families stand at three or more as of 2026-09-14: `Clock semantics` (26), `Concurrency` (22), `String` (16), `Atomic writes` (10), `Action wiring` (6), `Primitive policy` (4), `Indeterminate outcome` (4), `Composition state` (4), `Wiring decision` (3), `Instance` (3), `Expiry` (3). Standing as a candidate is not being promoted; Principle 8 governs, and Standard label 6 restates it here because a promotion is an admission.

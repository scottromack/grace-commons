# GRACE lang v0.41 — Minimal Earned Grammar
...
Terms › `term entry`: a Terms registry entry — a heading, prose and a `Kind` line — the reader's copy of a declaration (`spec-format.md` §*Terms*).

Terms › `bracket marker`: `[Name]` in prose, and the link line that lands it on the name's term entry; a pointer to the declaration, never a second copy of the declaration.
...
Terms › `category`: `actor` | `record` | `record verb` | `value set` | `bound` | `cadence` | `term` | `qualifier` | `cited` | `composing pattern`.

...
- **[State Machine](../atoms/state-machine.md)** — the step-sequence spine the position is brought to.
...
Composes 7: The composition MUST NOT read the spine for a position.
...
Term spine: the State Machine workflow serving a compensable workflow — the position's follower, never the position's source.
...
```
Composition state 1: The step log IS AUTHORITATIVE FOR a compensable workflow's position.
...
Term log projection: the position, the completed steps, the unresolved attempts, the compensation registry and the applied effects — each a derived index by the section titled Composition state in `execution-contract.md`, rebuilt by the replay and stored nowhere.

Term position: the phase, the current step and, once reached, the outcome.
...
Replay 14: The replay MUST produce one position PER step log.
```
...
position(compensable_workflow_id)
  answers position reading
...
Term advance result: the step the call acted on and the position the call left.
...
Term position reading: the position the replay derives — what [Position] answers.
...
Action wiring 78: [Position] MUST derive the position reading from the step log PER the replay.
...
Term section-taking action: [Advance] | [Cancel] | [Position].
...
Action wiring 78 is [Position] reading the log, never the spine, which may lawfully lag the log by one owed `fire`. [Read Log] is a passthrough to Event Log's `read`, the full step and compensation trail at any time.
...
- **Invariant 2 — Position equivalence.**
  ```
  Invariant 2.1: The spine's current state MUST equal the replayed position at EVERY quiescent point.
  Invariant 2.2: The composition MUST NOT claim an atomic pair across the spine AND the step log.
  Invariant 2.3: The composition MUST make the position-equivalence claim ONLY IF the deployment declares the constituent store durability AND the critical section.
  ```
...
1. `start_compensable_workflow(order_fulfillment, "order-9")` → `{compensable_workflow_id: s1}`. Log: `[compensable_workflow_started]`. Position: *not-started* (forward phase).
...
Check 2.2: An auditor MUST derive the position from the step log AND NOT from the spine (Composes 7).
...
Term terms: composition, constituents, compensable workflow, spine, step log, log projection, position, compensable workflow store, compensable workflow id, definition, definition reference, start instant, completed steps, unresolved attempts, compensation registry, applied effects, effect key, replay, standing halt, named step, resolving event, compensation order, compensation failure policy, step completion bound, critical section, lease length, constituent store durability, target, target key discipline, effect key horizon, longest attempt gap, captured input cap, supplied reason, horizon assertion, compensation inputs, overflowed step, oversized inputs, input envelope, captured inputs, largest admitted record, effect step, step marker, unmarked irreversible step, roll-forward step, truncation mark, input source, advance result, cancel result, position reading, indeterminate reason, advance failure position, cancel failure position, section-taking action, invocation, re-entry arm, owed transition, effect, compensation, attempt, effect verdict, forward advance, compensating advance, next step, pending compensation, standing failure, resumed attempt, completion record, marker, unrecorded effect, spine refusal, fault halt, finding, validated start, admitted start, halted advance, admitted cancel, event type, attempt marker, phase marker, terminal marker, resume marker, recorded horizon assertion, transition-bearing event, open obligation, recorded act, quiescent point.

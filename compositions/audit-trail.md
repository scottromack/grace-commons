...
```text
CP1: EXACTLY ONE Event Log instance MUST serve the composition.
CP2: EXACTLY ONE Actor Identity instance MUST serve the composition.
CP3: EXACTLY ONE Retention Window instance MUST serve the composition.
CP4: EXACTLY ONE Tamper Evidence instance MUST serve the composition.
CP5: The composition MUST call ActorIdentity.attest at [Record Action] step 2 alone.
CP6: The composition MUST call ActorIdentity.verify at [Verify Record] step 3 alone.
CP7: The composition MUST read an attestation's surviving fields through Actor Identity's declared read surface.
CP8: The composition MUST NOT delete an attestation.
CP9: The composition MUST call RetentionWindow.place_under_retention at [Record Action] step 4 and at the third half's compensating placement alone.
CP10: The composition MUST call RetentionWindow.purge at [Purge Event] step 1 alone.
CP11: [Purge Eligible] MUST delegate eligibility to RetentionWindow.purge_eligible.
CP12: The composition MUST NOT evaluate an eligibility clock.
CP13: The composition MUST call TamperEvidence.seal from [Seal Now] alone.
CP14: The composition MUST call TamperEvidence.verify at [Verify Record] step 5 alone.
CP15: The composition MUST NOT write an entry into the seal store other than through TamperEvidence.seal.
CP16: The composition MUST NOT dispose of a seal.
CP17: The composition MUST NOT seal a range some seal covers.
```
...
```text
CR1: The wired Event Log instance MUST expose the open-upper-bound read.
CR2: The deployment MUST alert on invalid-query from a composition-built query as a deployment fault.
CR3: The composition MUST NOT surface invalid-query from a composition-built query as an outcome of any action.
```
...
Terms › `composition-built query`: the open-upper-bound read, and the singleton range read of ES5 — queries the composition builds from its own indexes, which no caller input reaches.

WHY:
Eight procedures issue the read and none has a substitute — [Seal Now]'s head read, [Record Action] step 5's read-back, the rebuilds of `event_to_sequence`, `event_to_attestation`'s live entries and `compensated_attestations`, the scan's binding set and third half, and the auditor's own enumeration at GA2 — and the last is why the requirement is a deployment obligation: an instance that cannot serve it cannot be audited for orphan-freedom at all. No caller input reaches any of these queries (CR3).
...
```text
DI1: EVERY derived index MUST sit outside every action's atomicity surface.
DI2: A reader MUST consult a derived index with rebuild-on-miss.
DI3: The composition MUST treat a lost derived-index entry as a rebuild trigger.
DI4: The composition MUST NOT treat a lost derived-index entry as data loss.
DI5: A derived index MUST NOT claim cross-constituent transactional consistency.
DI6: The composition MUST NOT modify an inserted entry of an insert-only map.
DI7: The composition MUST NOT remove an inserted member of a closed-state marker.
```
...
- **`event_to_attestation`** — map from `event_id` to the `attestation_id` Actor Identity produced at record time; the auditor's traversal from an event to its attribution. The classification splits by retention state, because the rebuild's source does not survive the cascade.
  ```text
  EA1: [Record Action] step 5 MUST populate event_to_attestation with the event's event_id → attestation_id.
  EA2: WHEN retention_state != Purged:
      EA2a: The composition MUST classify the entry as derived index.
      EA2b: The rebuild MUST take, for EVERY event the full enumeration returns, the event's event_id as the key and the payload's attestation_id as the value.
  EA3: WHEN retention_state = Purged:
      EA3a: The composition MUST classify the entry as extraction-pending against Erasure Tombstone.
      EA3b: The destruction record MUST carry the pair (event_id, attestation_id).
      EA3c: The pair MUST carry the durability obligation of DU5.
  ```
...
- **`event_to_retention`** — map from `event_id` to the `retention_id` Retention Window produced at record time; the policy the event is held under.
  ```text
  ER1: [Record Action] step 5 MUST populate event_to_retention with the event's event_id → retention_id.
  ER2: The composition MUST classify event_to_retention as derived index.
  ER3: The rebuild MUST enumerate the retention store and re-key each retention record by the record's record_ref.
  ```
- **`event_to_sequence`** — map from `event_id` to the `sequence_number` Event Log assigned at append; the index that makes id-addressed reads possible.
  ```text
  ES1: [Record Action] step 5 MUST populate event_to_sequence with the event's event_id → sequence_number.
  ES2: The composition MUST classify event_to_sequence as derived index.
  ES3: The rebuild MUST re-key EVERY event the full enumeration returns by the event's own event_id.
  ES4: [Read Record], [Verify Record] and [Purge Event] MUST resolve a caller-supplied event_id through event_to_sequence.
  ES5: An id-addressed action MUST read the event by EventLog.read over the singleton range at the resolved sequence_number.
  ES6: The composition MUST NOT ask EventLog.read to select on event_id.
  ES7: The composition MUST NOT ask EventLog.read for a payload predicate.
  ```
...
- **`seal_coverage`** — for each `evidence_id` in the seal store, the contiguous sequence-number range the seal commits to as [Seal Now] cut it, plus a per-entry `purged_events` set the cascade writes; what tells the verifier which record set to present.
  ```text
  SC1: [Seal Now] MUST populate seal_coverage with evidence_id → the sealed slice.
  SC2: The composition MUST classify the ranges of seal_coverage as derived index.
  SC3: The rebuild of the ranges MUST enumerate the seal store and read each evidence record's record_set_ref.
  SC4: The rebuild of the ranges MUST NOT issue EventLog.read.
  SC5: A coverage range MUST key on sequence_number.
  SC6: A coverage range MUST NOT key on event_id.
  SC7: Two coverage ranges MUST NOT overlap.
  SC8: A coverage range MUST NOT carry a hole.
  SC9: The composition MUST classify purged_events as extraction-pending against Erasure Tombstone.
  SC10: The cascade MUST record the records-purged fact at event granularity.
  SC11: The cascade MUST NOT flag a whole seal_coverage entry as purged.
  SC12: purged_events MUST carry the durability obligation of DU5.
  ```
...
- **`erasure_outcomes`** — for each event over which [Purge Event] step 3 has issued its delegation, the outcome the configured `erasure_mechanism` reported: `destroyed` or `destruction-failed(reason)`.
  ```text
  EO1: The cascade MUST record the mechanism's reported outcome in erasure_outcomes for EVERY delegation [Purge Event] step 3 issues.
  EO2: erasure_outcomes MUST address an outcome per event_id.
  EO3: The composition MUST classify erasure_outcomes as extraction-pending against Erasure Tombstone.
  EO4: The outcome record MUST land on the event's destruction record.
  EO5: erasure_outcomes MUST carry the durability obligation of DU5.
  EO6: A destruction-failed record MUST NOT close an entry.
  ```
...
- **`compensated_attestations`** — the set of `attestation_id`s for which an `audit.compensation` event has been recorded; the closed-state marker for orphan reconciliation.
  ```text
  CA1: The composition MUST classify compensated_attestations as derived index.
  CA2: The rebuild MUST keep, from the full enumeration, EVERY event whose action_ref = audit.compensation AND whose payload subject = attestation, and take the attestation_id each payload names.
  CA3: The rebuild MUST filter on the subject-kind discriminator.
  CA4: The rebuild MUST filter in composition code.
  CA5: A deployment composing Reverse Index MAY filter through Reverse Index as an instance optimization.
  CA6: The rebuild MUST cover the audit.compensation events live in the log.
  ```
  Terms › `reconciled`: an orphan whose `attestation_id` is a member of `compensated_attestations`.

  WHY: the orphan never goes away — Actor Identity Invariant 9 forecloses deletion — so the store cannot carry the closure; the marker does. Without the subject filter a rebuild would read an `event_id` where it expected an `attestation_id`. Compensation events are themselves purged in time, so the rebuilt set covers the compensations still live (CA6); the residual — a second compensation for an orphan compensated a retention period earlier — is a duplicate, never a false record. The rebuild is trustworthy because the reserved namespace (PP7) means the set is read from records only the composition's own reconciliation path could have written.
- **`reported_beyond_horizon`** — the set of `attestation_id`s for which the scan's second half has recorded a *beyond the horizon* finding; the closed-state marker for that report.
  ```text
  RH1: The composition MUST classify reported_beyond_horizon as derived index.
  RH2: The rebuild MUST keep, from the full enumeration, EVERY event whose action_ref = audit.reconciliation AND whose payload subject = attestation AND whose payload disposition = beyond-horizon, and take the attestation_id each names.
  RH3: The rebuild MUST cover the audit.reconciliation events live in the log.
  ```
...
- **`retention_policy`** — a Retention Window `policy_ref`, or a policy selector `(action_ref, actor_ref, data) → policy_ref` for content-derived rules. *Default:* none.
  ```text
  RP1: The deployment MUST set retention_policy to the reconciled policy for the record class.
  RP2: [Record Action] step 4 MUST place the retention under the resolved policy.
  RP3: EXACTLY ONE retention record MUST govern an audit event and the event's attestation.
  RP4: The composition MUST NOT hold an attestation meta-retention policy.
  RP5: The composition MUST retain a seal indefinitely.
  ```
...
- **`seal_cadence`** — `per-event` | `interval-based` | `on-demand`; an interval-based cadence carries an events arm (every N events), a time arm (every T seconds), or both. *Default:* none.
  ```text
  SD1: The deployment MUST set seal_cadence from the forensic window the deployment's regime tolerates.
  SD2: A deployment whose auditor requires a bounded per-event tampering window MUST set per-event.
  ```
...
- **`seal_mechanism`** — a Tamper Evidence mechanism reference: hash chain, Merkle tree, RFC 3161-anchored timestamp. *Default:* none.
  ```text
  SM1: The deployment MUST select a mechanism sound for the full audit horizon.
  SM2: A deployment requiring the forensic-window bound of GA4 MUST select a chained mechanism.
  SM3: The deployment MUST set seal_mechanism to the mechanism the wired Tamper Evidence instance runs.
  SM4: The deployment MUST alert on a disagreement between seal_mechanism and the wired instance's mechanism as a deployment fault.
  SM5: The mechanism's seal-time rendering MUST match the verify-time presentation.
  SM5a: The deployment MUST declare the agreement of SM5.
  ```
...
- **`mechanism_credential`** — opaque credential material passed to `TamperEvidence.seal` on every seal, whatever the cadence: a signing key for a keyed mechanism, a TSA client credential for an anchored one. *Default:* empty (unkeyed).
  ```text
  MC1: The composition MUST pass mechanism_credential to TamperEvidence.seal on every seal.
  MC2: The composition MUST pass mechanism_credential through unchanged.
  MC3: The composition MUST NOT inspect mechanism_credential.
  MC4: The composition MUST NOT log mechanism_credential.
  MC5: IF mechanism class != unkeyed THEN the deployment MUST set mechanism_credential.
  MC6: IF mechanism class = unkeyed THEN the deployment MUST NOT set mechanism_credential.
  MC7: A deployment requiring non-repudiation of the seal MUST set a credential whose nature supplies non-repudiation.
  MC8: IF mechanism_credential NOT EXISTS THEN the composition MUST pass an empty credential.
  ```
...
- **`unsealed_tail_mode`** — `strict` | `lenient`: what [Verify Record] returns for an event in the unsealed tail. *Default:* `strict`.
  ```text
  UT1: A deployment MAY set lenient ONLY IF independently trusted substrate EXISTS AND standing false negative EXISTS.
  UT2: A regulated deployment MUST NOT set lenient.
  NOTE: watch applicability — the source's setting rules read *where X, do Y*; the applicability is carried in the subject here (UT2, SM2, RL3, MC7).
  UT3: IF unsealed_tail_mode NOT EXISTS THEN the instance MUST take strict.
  ```
...
- **`erasure_mechanism`** — a reference to the deployment's composed content-destruction mechanism, invoked at [Purge Event] step 3. *Default:* none.
  ```text
  EM1: The deployment MUST wire a shredding-class erasure mechanism.
  EM2: The deployment MUST NOT wire a tombstone-by-mutation mechanism.
  EM3: The erasure mechanism MUST destroy the readability of the whole of Event Log's data field.
  EM4: The erasure mechanism MUST destroy the readability of the attestation's proof.
  EM5: The erasure mechanism MUST NOT rewrite a stored field.
  EM6: The erasure mechanism MUST leave the attestation's surviving fields readable.
  EM7: The erasure mechanism MUST report EXACTLY ONE OF destroyed, destruction-failed(reason) for EVERY event the mechanism is asked about.
  EM8: The erasure mechanism MUST name the event_id in EVERY outcome.
  EM9: The deployment MUST evidence the mechanism's class to the auditor outside the records.
  ```
  ...
  WHY: the name `data` does double duty. The mechanism takes the outer object, so the payload's copies of `action_ref`, `actor_ref` and `attestation_id` go with it, which is why the purged-event answers read the attestation store; the *who / what / when* outlives the purge, the payload and the binding's verifiability do not. There is no safe default: defaulting to *no destruction* would leave *Purged* over readable content, the gap Invariant 8 forecloses. A mechanism that only reported *I was called* would make success undecidable (EM7), and an outcome not traceable to its event could close nothing (EM8). The class boundary is argued at Boundary one.
- **`compensation_window`** — the duration within which the liveness arms of Invariants 1, 2 and 8 close: an orphan attestation, an unretained event or a half-completed cascade is surfaced and reconciled inside it. *Default:* none.
  ```text
  CW1: The deployment MUST set compensation_window from the tightest reconciliation deadline the deployment's regime imposes.
  CW2: The composition MUST measure compensation_window from the finding's creation.
  CW3: The composition MUST NOT measure compensation_window from the finding's detection.
  CW4: An auditor MUST read the quiescence condition of GA2 from compensation_window.
  ```
  ...
  WHY: the window is the deployment's declared tolerance for a surfaced finding standing open — a regulatory judgment about its own regime. An orphan created at `t` is invisible to the scan until `t + bound + allowance`, the next run is at most one cadence later, and the closure lands one latency after that (IS16); a cadence no longer than the window is satisfied by a deployment that breaches on every orphan.
- **`reconciliation_cadence`** — how often the reconciliation scan runs. *Default:* the time arm of `seal_cadence` where that cadence carries one; none otherwise.
  ```text
  RD1: The reconciliation scan MUST run PER reconciliation_cadence.
  RD2: The reconciliation scan MUST run at restart.
  RD3: IF time arm NOT EXISTS THEN the deployment MUST set reconciliation_cadence.
  RD3a: IF time arm EXISTS AND reconciliation_cadence NOT EXISTS THEN the instance MUST take the time arm as reconciliation_cadence.
  RD4: reconciliation_cadence MUST govern all three halves of the scan.
  ```
...
- **`reconciliation_operator`** — an `actor_ref`: the deployment's maintenance actor authorized to record under the reserved `audit.*` namespace; the discriminator [Record Action] step 1's namespace gate turns on. *Default:* none.
  ```text
  RO1: The deployment MUST provision reconciliation_operator as an actor in the wired attestation store with usable credential material.
  RO2: [Record Action] step 1 MUST decide the reconciliation path by actor_ref = reconciliation_operator, byte-identity.
  ```
  WHY: without a declared discriminator the gate is undecidable — *external caller* names no observable property of a call — so an implementer could refuse `audit.*` from everyone (the liveness arms never close) or accept it from everyone (the closure marker is forgeable). Knowing the operator's `actor_ref` admits a caller past step 1 only; step 2's `attest` still demands the credential, so an `audit.*` event cannot exist in the log unless attested under the operator identity (GA7).
- **`reconciliation_operator_credential`** — opaque credential material for the actor `reconciliation_operator` names. *Default:* none.
  ```text
  OC1: The scan MUST hand reconciliation_operator_credential to ActorIdentity.attest on EVERY reconciliation-path write.
  OC2: The composition MUST NOT inspect reconciliation_operator_credential.
  OC3: The composition MUST NOT log reconciliation_operator_credential.
  OC4: The composition MUST NOT write reconciliation_operator_credential into a payload.
  ```
...
- **`payload_cap`** — the byte ceiling [Record Action] step 1 checks the full constructed payload against. *Default:* none.
  ```text
  PC1: The deployment MUST set payload_cap to the wired Event Log instance's own configured cap.
  PC2: [Record Action] step 1 MUST measure the serialized envelope against payload_cap.
  PC3: [Record Action] step 1 MUST NOT measure the sum of the members' own lengths.
  PC4: The deployment MUST alert on invalid-payload at [Record Action] step 3 as a deployment fault.
  ```
...
- **`reference_length_cap`** — a byte length, applied independently to `action_ref` and `actor_ref` at [Record Action] step 1. *Default:* 1 KB each.
  ```text
  RL1: [Record Action] step 1 MUST apply reference_length_cap to action_ref and to actor_ref independently.
  RL2: reference_length_cap MUST NOT EXCEED reference headroom.
  RL3: A deployment MAY raise reference_length_cap above 1 KB ONLY IF a genuine reference scheme EXCEEDS 1 KB.
  RL4: IF reference_length_cap NOT EXISTS THEN the instance MUST take 1 KB.
  ```
...
- **`attestation_id_width`** — the width of the `attestation_id` the configured Actor Identity instance allocates. *Default:* none.
  ```text
  AI1: The deployment MUST set attestation_id_width to the maximum width the wired Actor Identity instance allocates.
  AI2: [Record Action] step 1 MUST size the payload with attestation_id_width.
  ```
...
- **`record_action_completion_bound`** — the longest a [Record Action] may take between its first committed write (step 2's attestation, stamped `attested_at` at Actor Identity's seam) and its last (step 5's index writes). *Default:* none.
  ```text
  CB1: The deployment MUST set record_action_completion_bound from the observed worst-case latency of [Record Action] steps 2–5 with headroom, constituent round-trips included.
  ```
  WHY: a write issued inside the bound must also have landed inside it. The bound does three jobs, each stated where it happens: the lower edge of the scan's second and third halves (`record_edge`), the per-act lease length for a record action (PS9a), and the invocation's terminus (RA25).
- **`purge_completion_bound`** — the longest a [Purge Event] may take between step 1's transition (stamped `purged_at` at Retention Window's seam) and step 3's outcome record. *Default:* none.
  ```text
  PB1: The deployment MUST set purge_completion_bound from the observed worst-case latency of [Purge Event] steps 2–3 with headroom, the erasure mechanism's round-trip included.
  ```
...
- **`compensation_closure_latency`** — the deployment's disclosed bound on one whole closure landing, from the moment a scan half takes an act's section to the moment the closure's last record has landed. *Default:* none.
  ```text
  CL1: The deployment MUST declare compensation_closure_latency as the bound on one whole closure.
  ```
...
- **`clock_skew_allowance`** — the most the composition's seam clock and any constituent's seam clock may differ. *Default:* none.
  ```text
  CS1: The deployment MUST set clock_skew_allowance from the deployment's clock discipline.
  CS2: The scan MUST widen EVERY comparison of now with a constituent-stamped time by clock_skew_allowance.
  CS3: The scan MUST NOT decide a write by a cross-seam comparison alone.
  CS4: A cross-seam comparison MUST exclude a record from the pass and nothing more.
  ```
...
- **Per-act section** — an instance capability requirement, not a knob: a per-key critical section the host supplies, keyed by an act's id — the `attestation_id` [Record Action] step 2 returns for a record action, the `event_id` for a cascade. *Default:* none.
  ```text
  PS1: The host MUST supply a per-key critical section keyed by the act's id.
  PS2: The host MUST release the section on the holder's return.
  PS3: The host MUST release the section on the holder's death.
  PS4: A host that cannot detect the holder's death MUST implement the section as a lease.
  PS5: A leg that finds an act's section held MUST skip the act for the rest of the run.
  PS6: A leg MUST NOT block on a held section.
  PS7: IF held section NOT EXISTS THEN the invocation MUST NOT issue a later write.
  NOTE: watch condition negation — *not holding the section* is written as a minted term's NOT EXISTS (PS7, PS8, IV25).
  PS8: IF held section NOT EXISTS THEN the invocation MUST NOT land a later write.
  PS9: WHEN section_kind = lease:
      PS9a: The host MUST set the lease to the act's completion bound.
      PS9b: IF lease = expired THEN the invocation MUST NOT issue a truth-bearing write.
      PS9c: An invocation whose truth-bearing writes have all landed MUST complete the invocation's index writes and return success.
  PS10: WHEN section_kind = death-detected:
      PS10a: The invocation MUST NOT read a clock.
      PS10b: The invocation MUST take proceed as landed as the invocation's terminus.
  PS11: A writer without the section MUST NOT read a pre-check BEFORE re-taking the section.
  PS12: A writer MUST re-read the pre-check under the re-taken section.
  ```
  ...
  WHY: one-writer-per-act rests on the section. A held section is a live invocation inside its bound, and the age edge already keeps a leg off work that young, so a leg skips rather than waits. A record action's key is the `attestation_id` because no `event_id` exists before step 3, and a section that began there would leave the append outside it. Where the section is a lease, expiry is the terminus (RA25, PU12); where it is not, the terminus is the pre-check-and-adopt of step 4 or steps 2–3.

### Instance start

```text
IS1: IF retention_policy NOT EXISTS THEN the instance MUST NOT start.
IS2: IF seal_cadence NOT EXISTS THEN the instance MUST NOT start.
IS3: IF seal_mechanism NOT EXISTS THEN the instance MUST NOT start.
IS4: IF erasure_mechanism NOT EXISTS THEN the instance MUST NOT start.
IS5: IF compensation_window NOT EXISTS THEN the instance MUST NOT start.
IS6: IF reconciliation_cadence NOT EXISTS THEN the instance MUST NOT start.
IS7: IF reconciliation_operator NOT EXISTS THEN the instance MUST NOT start.
IS8: IF reconciliation_operator_credential NOT EXISTS THEN the instance MUST NOT start.
IS9: IF payload_cap NOT EXISTS THEN the instance MUST NOT start.
IS10: IF attestation_id_width NOT EXISTS THEN the instance MUST NOT start.
IS11: IF record_action_completion_bound NOT EXISTS THEN the instance MUST NOT start.
IS12: IF purge_completion_bound NOT EXISTS THEN the instance MUST NOT start.
IS13: IF compensation_closure_latency NOT EXISTS THEN the instance MUST NOT start.
IS14: IF clock_skew_allowance NOT EXISTS THEN the instance MUST NOT start.
IS15: IF the per-act section NOT EXISTS THEN the instance MUST NOT start.
IS16: The instance MAY start ONLY IF compensation_window EXCEEDS closure_sum.
IS17: The instance MUST read closure_sum's four terms at start.
```
...
```text
PP1: action_ref MUST contain a non-whitespace character.
PP2: actor_ref MUST contain a non-whitespace character.
PP3: [Record Action] step 1 MUST validate action_ref and actor_ref at this layer.
PP3a: [Record Action] MUST NOT call a constituent BEFORE step 1 completes.
PP4: [Record Action] step 1 MUST land rejected(invalid-request) for a malformed reference, with nothing recorded.
PP5: The composition MUST NOT normalize any input.
PP6: The composition MUST compare references by byte-identity.
PP7: [Record Action] step 1 MUST land rejected(invalid-request) for an action_ref whose bytes begin with the prefix audit. from a caller whose actor_ref != reconciliation_operator, with nothing recorded.
PP8: The reconciliation path MAY record under the reserved namespace ONLY IF the action_ref = audit.compensation OR the action_ref = audit.reconciliation.
PP9: The composition MUST consume credential through ActorIdentity.attest alone.
PP10: The composition MUST NOT inspect credential.
PP11: The composition MUST NOT store credential.
PP12: The composition MUST NOT write credential into a payload.
PP12a: The composition MUST NOT write credential into a log line.
PP13: The composition MUST NOT apply a length cap to credential.
PP14: The composition MUST NOT read inside a caller's data.
PP15: A policy selector MAY inspect data at [Record Action] step 4's policy resolution.
PP16: The composition MUST NOT retain what the selector reads.
PP16a: The composition MUST NOT log what the selector reads.
PP17: The composition MAY read inside a payload ONLY IF the payload's action_ref = audit.compensation OR the payload's action_ref = audit.reconciliation.
PP18: [Record Action] step 1 MUST measure the serialized envelope of the full constructed payload against payload_cap, sized with attestation_id_width.
PP19: [Record Action] step 1 MUST land rejected(invalid-request) for an oversize payload, with nothing recorded.
PP20: An empty data MUST count as valid.
PP21: An unknown event_id MUST yield not-known from the addressed action.
PP22: The composition MUST pass original_event_payload through to TamperEvidence.verify unchanged.
PP23: original_event_payload MUST match the verify-time presentation.
PP24: The caller MUST present a non-empty original_event_payload.
PP25: The composition MUST NOT canonicalize original_event_payload.
```
...
```text
AW1: The composition MUST pass a query by sequence-number range through to EventLog.read unchanged.
AW2: The composition MUST pass a query by wall-time range through to EventLog.read unchanged.
AW3: The composition MUST NOT serve a query by payload field.
AW4: A deployment needing a query by payload field MUST compose Reverse Index over the audit log and join the results through [Read Record] one event_id at a time.
```
...
```text
RA1: The recording-failure arm MUST carry the step that refused.
```
...
1. **Validate the primitives at this layer, before any constituent is called.**
   ```text
   RA2: [Record Action] step 1 MUST validate action_ref and actor_ref per PP1–PP4 and PP7.
   RA3: [Record Action] step 1 MUST size the full constructed payload per PP18 and PP19.
   RA4: A step-1 refusal MUST record nothing.
   RA5: WHEN actor_ref = reconciliation_operator:
       RA5a: [Record Action] step 1 MUST land rejected(invalid-request) for an audit.compensation payload carrying no subject-kind discriminator.
       RA5b: [Record Action] step 1 MUST land rejected(invalid-request) for an audit.compensation payload carrying no id for the subject.
   RA6: [Record Action] step 1 MUST NOT validate the shape of a payload written outside the reserved namespace.
   ```
   ...
   WHY: the size check sits ahead of step 2 so an oversized payload can never strand a committed, immutable attestation. The namespace check has an external side — any call whose `actor_ref` is not the operator's, there being no other marker of origin — and an internal side, RA5's payload requirement; a caller supplying the operator's `actor_ref` without the credential passes here and is refused at step 2 with nothing recorded. This is the one place the composition validates a payload's shape, and only of payloads it wrote itself.
2. **Attest.**
   ```text
   RA7: [Record Action] step 2 MUST call ActorIdentity.attest(action_ref, actor_ref, credential) → attestation_id.
   RA8: [Record Action] step 2 MUST land invalid-credential as rejected(invalid-credential).
   RA9: [Record Action] step 2 MUST land invalid-request as rejected(invalid-request).
   RA10: [Record Action] step 2 MUST land storage-failure as [Recording Failure].
   RA11: A step-2 refusal MUST record nothing further.
   RA12: [Record Action] MUST take the per-act section on the attestation_id step 2 returned.
   RA13: [Record Action] MUST hold the section through step 5.
   RA14: [Record Action] MUST release the section on return.
   ```
...
3. **Append.**
   ```text
   RA15: [Record Action] step 3 MUST call EventLog.append with the full constructed payload → event_id.
   RA16: The composition MUST NOT supply recorded_at.
   RA17: [Record Action] step 3 MUST land storage-failure as [Recording Failure].
   RA18: [Record Action] step 3 MUST land invalid-payload as rejected(invalid-request).
   ```
   WHY: Event Log stamps `recorded_at` at its own seam from the host-injected clock, and that stamp is the audit event's timestamp wherever it is read back; a business event-time lives inside the opaque `data`. `invalid-payload` is reachable — `data` is caller-supplied and Event Log enforces a cap — which is why step 1 sizes first; reaching the arm after step 1 passed means `payload_cap` and the wired instance's cap disagree, a deployment fault (PC4), not a caller rejection.
4. **Place under retention.**
   ```text
   RA19: [Record Action] step 4 MUST NOT place a retention BEFORE re-reading event_to_retention for the event_id under the section.
   RA20: IF a retention for the event_id EXISTS THEN [Record Action] step 4 MUST adopt the retention as landed and continue to step 5.
   RA21: [Record Action] step 4 MUST call RetentionWindow.place_under_retention(event_id, resolved policy) → retention_id, with record_ref = event_id.
   RA22: IF lease = expired THEN [Record Action] step 4 MUST NOT place.
   RA23: [Record Action] step 4 MUST land invalid-request as rejected(invalid-request).
   RA24: [Record Action] step 4 MUST land invalid-policy and policy-not-found as rejected(invalid-request).
   RA24a: The deployment MUST alert on invalid-policy and policy-not-found as a deployment fault.
   RA24b: [Record Action] step 4 MUST land storage-failure as [Recording Failure].
   ```
   WHY: the third half takes the same section — on the event's payload `attestation_id` — before it pre-checks `event_to_retention` and places, and examines no event younger than `record_edge`, so this placement and the leg's are never both made for one event (CC3, TH4). Where the lease has expired before this step issues, the event is the third half's from here on and a later placement is the leg's. The policy reference came from Configuration, not the caller, so `invalid-policy` and `policy-not-found` are deployment faults surfaced on the caller's arm.
5. **Record the indexes.**
   ```text
   RA26: [Record Action] step 5 MUST record event_to_attestation, event_to_retention and event_to_sequence for the event.
   RA27: [Record Action] step 5 MUST take sequence_number from the read-back.
   RA28: The read-back MUST run from the high-water mark to the open end.
   RA29: The read-back MUST match the returned event_id by equality.
   RA30: IF the read-back returns no event carrying the event_id THEN [Record Action] step 5 MUST fall back to the rebuild of event_to_sequence.
   RA31: [Record Action] step 5 MUST NOT guess a sequence_number.
   RA32: An implementation whose append result surfaces sequence_number MAY take the value from the append result and skip the read-back.
   RA33: The composition MUST NOT depend on the append result surfacing sequence_number.
   RA34: The composition MUST treat a step-5 failure as a rebuild trigger.
   RA35: A step-5 failure MUST NOT land [Recording Failure].
   ```
...
6. **Seal under per-event cadence.**
   ```text
   RA36: IF seal_cadence = per-event THEN [Record Action] step 6 MUST seal through [Seal Now].
   RA37: IF seal_cadence != per-event THEN [Record Action] step 6 MUST defer sealing to the next cadence firing.
   RA38: A cadence firing MUST seal the slice.
   RA39: A seal failure at step 6 MUST NOT reject [Record Action].
   RA40: The deployment MUST alert on a step-6 seal failure with the cause.
   RA41: The next cadence firing MUST retry the seal.
   ```
...
7. **Return.**
   ```text
   RA42: [Record Action] step 7 MUST return event_id.
   RA43: IF step-3 storage failure EXISTS THEN [Record Action] MUST return rejected(recording-failure(step-3)).
   NOTE: watch event versus state — *step 3 refusing after step 2 committed* is an event, written as a minted term's EXISTS (RA43, RA44); *[Seal Now] rejects* is written as a bare condition (SN9–SN11, PU10).
   RA44: IF step-4 storage failure EXISTS THEN [Record Action] MUST return rejected(recording-failure(step-4)).
   RA43a: A recording-failure outcome MUST surface the partial state the invocation left.
   RA45: [Record Action] MUST NOT land a non-storage refusal of steps 3–4 as [Recording Failure].
   RA25: WHEN mid-record expiry EXISTS:
       RA25a: [Record Action] MUST NOT issue a further constituent write.
       RA25b: [Record Action] MUST return rejected(recording-failure(step)) naming the first step not completed.
       RA25c: [Record Action] MUST release the section.
   RA46: An invocation past step 4 at the bound MUST complete step 5 and return event_id.
   RA47: The invocation MUST NOT write a compensation for the partial state the invocation left.
   ```
   ...
   WHY: step 3 failing after step 2 leaves an orphan attestation, closed by Invariant 1's liveness arm and found by the binding-set half; step 4 failing after step 3 leaves an unretained event, closed by Invariant 2's arm and found by the third half. The two are not interchangeable, and which one the operator is looking at is read off the step. RA45's refusals leave the same partial state but are input or deployment faults, and collapsing them into `recording-failure` would misname the cause. RA25b names `step-3` or `step-4`, never `step-5`; a later success is the leg's. On a death-detected host the terminus is step 4's pre-check-and-adopt (PS10).
...
```text
SN1: [Seal Now] MUST read tail by the open-upper-bound read beginning at the slice's first sequence_number.
SN2: [Seal Now] MUST NOT read next_sequence_number.
SN3: IF the tail read returns no event THEN [Seal Now] MUST land [Nothing To Seal].
SN4: [Seal Now] MUST call TamperEvidence.seal(slice_ref, mechanism_credential) over the slice.
SN5: [Seal Now] MUST record the seal_coverage entry for evidence_id as the slice and advance sealed_through to tail.
SN6: [Seal Now] MUST land mechanism-failure(reason) as [Mechanism Failure] carrying the reason unchanged.
SN7: [Seal Now] MUST land invalid-request as rejected(invalid-request).
SN8: [Seal Now] MUST land storage-failure as [Recording Failure].
SN9: IF [Seal Now] rejects THEN sealed_through MUST NOT advance.
SN10: IF [Seal Now] rejects THEN [Seal Now] MUST NOT write a seal_coverage entry.
SN11: IF [Seal Now] rejects THEN the next cadence firing MUST re-seal the slice.
SN12: The composition MUST NOT expose a re-sealing surface.
```
...
```text
RR1: [Read Record] MUST NOT change state.
RR2: [Read Record] MUST NOT produce an audit event.
RR3: [Read Record] MUST NOT carry a permissions layer at this composition.
RR15: [Read Record] MUST NOT verify.
```
...
1. **Read the retention record.**
   ```text
   RR4: [Read Record] step 1 MUST read the retention record through event_to_retention.
   ```
2. **Read the event.**
   ```text
   RR5: [Read Record] step 2 MUST read the event per ES4 and ES5.
   ```
...
3. **Decide `not-known`.**
   ```text
   RR6: IF retention record NOT EXISTS AND log entry NOT EXISTS THEN [Read Record] step 3 MUST land not-known.
   RR6a: [Read Record] MUST NOT land not-known for an event_id with a log entry.
   RR7: The deployment MUST alert on a retention record whose event_id resolves to no log entry as a deployment fault.
   ```
...
4. **Assemble the [Audit Record].**
   ```text
   RR8: [Read Record] step 4 MUST assemble and return the audit record.
   RR9: [Read Record] step 4 MUST report coverage status per event.
   RR10: [Read Record] step 4 MUST return the covering seal's full range.
   RR11: WHEN retention_state = Purged:
       RR11a: [Read Record] step 4 MUST read action_ref, actor_ref and attested_at from the attestation store through the pair.
       RR11b: [Read Record] step 4 MUST NOT read the who / what / when from the event payload.
       RR11c: [Read Record] step 4 MUST return no data.
       RR11d: IF pair NOT EXISTS THEN [Read Record] step 4 MUST return the audit record with attribution not-recoverable, the retention record in Purged with purged_at, the coverage status, sequence_number and recorded_at.
       RR11e: IF pair NOT EXISTS THEN [Read Record] step 4 MUST NOT land not-known.
   ```
...
6. **No verification** (RR15). [Read Record] presents; [Verify Record] proves. The split is forced by Invariant 7 — verification needs the original payload re-presented, and a read surface that fetched it internally would destroy the asymmetry the composition exists to surface.
...
```text
VR1: The qualifier (compensation-window) MUST ride a separate channel beside the outcome.
VR2: The composition MUST NOT fold the qualifier into a reason.
VR3: The composition MUST NOT promote the qualifier to an outcome.
VR4: not-known MUST NOT carry the qualifier.
```
...
1. **Retention state first.**
   ```text
   VR5: [Verify Record] step 1 MUST read the retention record through event_to_retention.
   VR6: IF retention_state = Purged THEN [Verify Record] step 1 MUST land failed-verification(purged).
   VR7: [Verify Record] MUST NOT run step 3 for a purged event.
   ```
...
2. **Log presence.**
   ```text
   VR8: [Verify Record] step 2 MUST read the event per ES4 and ES5.
   VR9: IF retention record NOT EXISTS AND log entry NOT EXISTS THEN [Verify Record] step 2 MUST land not-known.
   VR10: IF log entry EXISTS AND retention record NOT EXISTS THEN [Verify Record] MUST proceed to step 3 and carry the qualifier on the outcome.
   ```
...
3. **Attestation.**
   ```text
   VR11: [Verify Record] step 3 MUST call ActorIdentity.verify on event_to_attestation's entry for the event_id.
   VR12: [Verify Record] step 3 MUST land a failed-verification(reason) with the reason prefixed attestation-.
   VR13: [Verify Record] step 3 MUST route registry-unavailable to step 6.
   VR14: [Verify Record] step 3 MUST land Actor Identity's not-known as failed-verification(attestation-not-known).
   VR15: IF step 3 lands failed-verification THEN [Verify Record] MUST return the step-3 outcome.
   VR15a: IF step 3 lands failed-verification THEN [Verify Record] MUST NOT run steps 4, 5 and 6.
   VR16: IF step 3 routes registry-unavailable THEN [Verify Record] MUST NOT run steps 4 and 5.
   VR17: [Verify Record] MAY run steps 4 and 5 ONLY IF step 3 landed verified.
   ```
...
4. **Locate the covering seal.**
   ```text
   VR18: [Verify Record] step 4 MUST locate the covering seal.
   VR19: IF partly-purged coverage EXISTS THEN [Verify Record] step 4 MUST land unverifiable(partially-purged-coverage).
   VR19a: IF partly-purged coverage EXISTS THEN [Verify Record] MUST NOT run step 5.
   VR20: IF covering seal NOT EXISTS AND unsealed_tail_mode = strict THEN [Verify Record] step 4 MUST land failed-verification(unsealed).
   VR21: WHEN covering seal NOT EXISTS AND unsealed_tail_mode = lenient:
       VR21a: [Verify Record] step 4 MUST treat coverage as satisfied.
       VR21b: [Verify Record] MUST skip step 5 and proceed to step 6.
   ```
...
5. **Seal.**
   ```text
   VR22: [Verify Record] step 5 MUST call TamperEvidence.verify(evidence_id, original_event_payload).
   VR23: [Verify Record] step 5 MUST land a failed-verification(reason) with the reason prefixed seal-.
   VR24: [Verify Record] step 5 MUST route mechanism-verification-unavailable to step 6.
   VR25: [Verify Record] step 5 MUST land Tamper Evidence's not-known as failed-verification(seal-not-known).
   ```
   WHY: the verifier presents what the covering seal commits to (PP23): under per-event cadence typically this one event's payload; under interval-based or on-demand cadence the payloads of the seal's whole range, obtained through Event Log's range read over the range [Read Record] reported. An absent, short or wrong presentation surfaces as `failed-verification(seal-record-set-mismatch)` from Tamper Evidence.
6. **Availability arm.**
   ```text
   VR26: [Verify Record] step 6 MUST land unverifiable(attestation-registry-unavailable) for registry-unavailable.
   VR27: [Verify Record] step 6 MUST land unverifiable(seal-mechanism-verification-unavailable) for mechanism-verification-unavailable.
   ```
...
7. **Verified.**
   ...
   ```text
   VR28: IF standing finding NOT EXISTS THEN [Verify Record] step 7 MUST land verified.
   VR29: IF step 2 established the compensation window THEN the returned outcome MUST carry the qualifier.
   ```
...
```text
PE1: [Purge Eligible] MUST delegate eligibility to RetentionWindow.purge_eligible.
PE2: [Purge Eligible] MUST map each eligible retention_id to the event_id through the retention record's record_ref.
PE3: [Purge Eligible] MUST NOT re-derive eligibility.
PE4: [Purge Eligible] MUST NOT share the projection's now reading with a later RetentionWindow.purge.
PE5: [Purge Eligible] MUST NOT reject.
PE6: [Purge Eligible] MUST NOT filter by legal hold.
```
...
```text
PU1: IF retention record NOT EXISTS AND log entry NOT EXISTS THEN [Purge Event] MUST land rejected(not-known).
PU2: [Cascade Failure] MUST carry EXACTLY ONE OF seal, step-1, step-2, step-3.
PU3: WHEN cascade-failure = seal OR cascade-failure = step-1:
    PU3a: The cascade MUST leave the retention in Retained.
    PU3b: The cascade MUST write nothing.
    PU3c: [Purge Eligible] MUST re-offer the event.
PU4: WHEN cascade-failure = step-2 OR cascade-failure = step-3:
    PU4a: The reconciliation scan MUST re-drive the entry.
    PU4b: The deployment MUST surface an open entry as a compliance alert.
    NOTE: watch persistent state — the source says *until the scan closes it*; the duration is carried by the term open entry (PU4b), by *for the rest of the run* (PS5) and by *through the outage* (LV1b).
PU5: WHEN Legal Hold = composed:
    PU5a: [Purge Event] MUST carry the under-legal-hold arm.
    PU5b: IF hold EXISTS THEN [Purge Event] MUST land rejected(under-legal-hold) with no cascade step executed.
PU6: IF Legal Hold != composed THEN [Purge Event] MUST NOT carry the under-legal-hold arm.
PU12: WHEN mid-cascade expiry EXISTS:
    PU12a: [Purge Event] MUST NOT issue a further write.
    PU12b: [Purge Event] MUST return rejected(cascade-failure(step)) naming the first step not completed.
    PU12c: The reconciliation scan MUST re-drive the cascade.
```
...
0. **Seal before purge.**
   ```text
   PU7: [Purge Event] step 0 MUST resolve event_to_sequence for the event_id.
   PU8: IF the event's sequence_number EXCEEDS sealed_through THEN [Purge Event] step 0 MUST invoke [Seal Now].
   PU9: [Purge Event] MUST NOT run step 1 BEFORE the covering seal exists.
   PU10: IF [Seal Now] rejects THEN [Purge Event] MUST land rejected(cascade-failure(seal)).
   PU11: IF the covering seal EXISTS THEN [Purge Event] step 0 MUST NOT invoke [Seal Now].
   ```
   ...
   **Step 0½ — retention must be resolved before the cascade proper begins.**
   ```text
   PU13: [Purge Event] MUST NOT run step 0½ BEFORE step 0.
   PU14: IF log entry EXISTS AND retention record NOT EXISTS THEN [Purge Event] step 0½ MUST land rejected(retention-unresolved).
   PU15: A retention-unresolved refusal MUST run no cascade step.
   PU15a: A retention-unresolved refusal MUST leave step 0's seal standing.
   PU16: The composition MUST NOT invent a retention in order to expire an event.
   ```
...
1. **Purge the retention.**
   ```text
   PU17: [Purge Event] step 1 MUST call RetentionWindow.purge(retention_id).
   PU18: [Purge Event] step 1 MUST land retention-period-not-elapsed as [Not Eligible].
   PU19: [Purge Event] step 1 MUST land not-known as rejected(not-known).
   PU20: IF step 1 lands not-retained THEN the cascade MUST resume from step 2.
   PU21: [Purge Event] step 1 MUST land storage-failure as rejected(cascade-failure(step-1)).
   ```
   WHY: the one constituent with a purge surface, and even it deletes no record — `purge` is a state transition, the record survives in *Purged* with `purged_at`, and that surviving record is the evidence the destruction was lawful. PU20 is what makes a re-driven or retried cascade idempotent.
2. **Write the destruction record, before anything is destroyed.**
   ```text
   PU22: [Purge Event] step 2 MUST write the destruction record in one durable write.
   PU23: The destruction record MUST add the event's sequence_number to the covering seal's purged_events.
   PU24: The destruction record MUST carry the pair, read from event_to_attestation with rebuild-on-miss.
   PU25: [Purge Event] MUST NOT issue the delegation BEFORE the destruction record has landed.
   PU26: A step-2 failure MUST land rejected(cascade-failure(step-2)).
   ```
   WHY: the binding lives inside the payload and step 3 destroys the payload's recoverability, so a cascade that captured the pair after the delegation would read a binding that no longer exists; capturing it here keeps the traversal *purged event → the attestation destroyed with it* possible at all. Both facts are extraction-pending truth against Erasure Tombstone, and this write is the part of the cascade carrying a durability obligation (DU5). The seal record itself is retained indefinitely as evidence that the records existed and were sealed before they were destroyed.
3. **Delegate the destruction.**
   ```text
   PU27: [Purge Event] step 3 MUST delegate destruction to erasure_mechanism.
   PU28: The cascade MUST NOT destroy content.
   PU29: The delegation MUST name the whole of Event Log's data field and the attestation's proof, and nothing else.
   PU30: [Purge Event] step 3 MUST record the mechanism's outcome in erasure_outcomes.
   PU31: IF outcome = destruction-failed THEN [Purge Event] step 3 MUST land rejected(cascade-failure(step-3)).
   PU32: A delegation not issued MUST land rejected(cascade-failure(step-3)).
   PU33: An outcome not recorded MUST land rejected(cascade-failure(step-3)).
   PU34: The cascade MUST NOT record a destroyed outcome the mechanism did not report.
   PU35: The composition MUST NOT repair a partly-purged seal.
   PU36: A partly-purged seal MUST remain the one seal over the seal's range.
   ```
...
4. **What survives a completed cascade.**
   ```text
   PU37: A completed cascade MUST leave standing the retention record in Purged with purged_at, the seal record with the event's sequence_number in purged_events, the destruction record's pair, the destroyed outcome in erasure_outcomes, and the attestation's surviving fields.
   PU38: The delegation MUST cover the event and the attestation in one cascade under one retention_id.
   ```
...
The composition's load-bearing wiring decision: when an event's retention elapses, the composition first ensures the event is covered by a seal (PU7–PU11), then purges the one record it is permitted to purge — the Retention Window record (PU17) — writes a destruction record adding the event's sequence number to the covering seal's `purged_events` set and capturing the event's `(event_id, attestation_id)` binding before it is destroyed (PU22–PU25), and delegates destruction of Event Log's `data` field and the attestation's `proof` to the deployment's shredding-class erasure mechanism (PU27–PU30). It coordinates the cascade and records the outcome; it destroys nothing itself (PU28), disposes of no seal (CP16), and re-seals nothing (SN12).
...
The class boundary's home; EM1–EM9, [Purge Event] step 3, Invariant 5 and the [Purge Event] card cite it.

```text
BO1: [Purge Event] MUST NOT write the destruction record BEFORE step 1's transition has committed.
BO2: The composition MUST NOT condition RetentionWindow.purge's outcome on the erasure mechanism's answer.
BO3: The composition MUST record a divergence between a Purged retention and readable content.
BO4: The composition MUST surface a divergence as a compliance alert.
BO5: The composition MUST bound a divergence by compensation_window.
BO6: The first half MUST re-drive a divergence on EVERY run.
```
...
WHY:
A conforming mechanism destroys the key material under which the content was stored and leaves every stored field as written, which is why Event Log Invariant 2 and Actor Identity Invariant 1 — immutability claims over stored fields, neither claiming readability — survive the cascade verbatim; tombstone-by-mutation breaks exactly those two (EM2). Erasure Tombstone records destruction as a new write-once record and never mutates the record it describes. One constituent prescription is declined by name: Retention Window's *Divergence between retention state and underlying record* edge case has `purge` return `storage-failure` when destruction cannot be confirmed. This composition cannot take it — the destruction record must be written after the transition and before the delegation (BO1, PU25), and the destruction is performed by a mechanism `RetentionWindow.purge` has no view of — so the divergence is recorded, surfaced, bounded and re-driven rather than prevented (BO3–BO6), the trade that buys the durable pre-destruction record. The terminus of BO6's loop is the open question Ledger line 2026-08-30-d names.

#### Boundary two — the composition disposes of no seal and writes no replacement

```text
BO7: EVERY seal MUST remain the only seal over the seal's range.
BO8: IF seal disposal EXISTS THEN the deployment MUST compose Erasure Tombstone for the seal-disposal surface and the coverage bookkeeping.
BO9: IF re-sealing EXISTS THEN the deployment MUST compose Seal Lifecycle.
BO10: The composition MUST NOT carry a which-seal-is-current fact.
```
...
```text
RS1: The reconciliation scan MUST run a first half over Purged retentions, a second half over the attestation store, and a third half over the audit log.
RS2: The scan MUST surface EVERY unreconciled finding as a compliance alert.
RS3: The scan MUST NOT carry an unreconciled finding silently.
RS4: The scan MUST close EVERY finding WITHIN compensation_window of the finding's creation.
NOTE: watch satisfaction — what a run that misses WITHIN is (RS4, IV4, IV11) is decided by LV1 for an outage and by GA2 otherwise; the language says nothing.
RS5: The scan MUST read now once per run at the scan's own seam.
RS6: The scan MUST NOT write BEFORE taking the per-act section for the act.
RS7: EVERY half MUST decide a write by the half's own predicate, read under the section.
```
...
WHY:
The halves share a trigger (RD1, RD2), a surfacing discipline (RS2, RS3), a deadline measured from the finding's creation (CW2, RS4), and two edges. Below, each half examines nothing younger than its completion bound widened by the allowance, because each stamp was written at a constituent's seam and the scan's `now` is read once at its own (RS5, CS2); the comparison only excludes a record from the pass (CS4). Above, the horizon is the point past which the compensation event a half would look for has been lawfully purged, and there the half reports rather than repairs.

*First half — the half-completed cascade.*

```text
FH1: The first half MUST enumerate Purged retentions.
FH2: IF purge_edge EXCEEDS purge age THEN the first half MUST NOT examine the retention.
FH3: The first half MUST test EVERY examined retention for a purged_events membership, a pair in the destruction record, and a destroyed outcome in erasure_outcomes.
FH4: The first half MUST test for a destroyed outcome.
FH5: The first half MUST re-drive a retention failing FH3 through [Purge Event].
FH6: The first half MUST NOT re-read the predicate BEFORE taking the section keyed by event_id.
FH7: The first half MUST NOT write BEFORE re-reading the predicate under the section.
FH8: The first half MUST re-drive EVERY examined open entry on EVERY run.
```

WHY: a crash between the cascade's steps is reachable, and the state is invisible to [Purge Eligible], whose projection is false for a *Purged* retention — the retry can never be driven from the eligible list. A cascade still inside its bound is in flight, not half-completed (FH2). Presence of an outcome record is not the test (FH4): an entry carrying `destruction-failed` is a retention saying *lawfully destroyed* over readable content, and a predicate that accepted any outcome would close it on the evidence of its own failure. Two scan runs, or a run and a cascade short of its bound, never both drive one entry (FH6, FH7).

*Second half — the orphan-attestation predicate.*

```text
SH1: The second half MUST build the binding set.
SH2: The second half MUST treat an attestation absent from the binding set as an orphan.
SH3: IF record_edge EXCEEDS attestation age THEN the second half MUST NOT examine the attestation.
SH4: IF attestation age EXCEEDS horizon THEN the second half MUST report the orphan as beyond the horizon.
SH5: IF attestation age EXCEEDS horizon THEN the second half MUST NOT write a compensation.
SH6: A beyond-horizon report MUST carry subject = attestation, the attestation_id and disposition = beyond-horizon, as an audit.reconciliation record.
SH7: The second half MUST NOT report BEFORE reading reported_beyond_horizon under the section.
SH7a: The second half MUST NOT report an orphan whose attestation_id is a member of reported_beyond_horizon.
SH8: The second half MUST NOT write a compensation BEFORE reading compensated_attestations under the section keyed by the orphan's attestation_id.
SH9: The second half MUST NOT write a compensation for a reconciled orphan.
SH10: The second half MUST hold the section through the compensating write.
SH11: A writer other than the scan MUST NOT write an orphan's compensation.
SH12: The second half MUST write EXACTLY ONE compensation per orphan.
```
...
WHY: *an attestation with no event* is not a fact any single store holds, so the set needs both enumerations: the first alone would report every lawfully purged event's attestation as an orphan the day its payload was shredded, the second alone would see nothing but purges — GA7's live/purged split read in the other direction. An attestation younger than `record_edge` may be a [Record Action] between steps 2 and 3, and a compensation for it would be a false record the seal then protects (SH3). Past the horizon the compensation, had it been written, has been purged out of the rebuild, so the half reports once (SH4–SH7). The orphan is permanent, so without SH8 a scan would compensate it every cadence forever; the pair under the section is what makes it a decision rather than a race (CC4). The invocation writes nothing (RA47), so a stalled invocation waking after the scan has nothing left to write.

*Third half — the unretained-event predicate.*

```text
TH1: The third half MUST enumerate the audit log by the full enumeration.
TH2: The third half MUST test EVERY returned event against event_to_retention with rebuild-on-miss.
TH3: The third half MUST NOT conclude a missing retention BEFORE rebuilding event_to_retention.
TH4: IF record_edge EXCEEDS event age THEN the third half MUST NOT examine the event.
TH5: The third half MUST take the section keyed by the event's payload attestation_id for a true miss.
TH6: The third half MUST NOT place BEFORE re-reading event_to_retention under the section.
TH7: The third half MUST hold the section through the placement.
TH8: The third half MUST NOT place BEFORE the audit.reconciliation intent has landed.
TH9: The third half MUST NOT write the audit.compensation record BEFORE the placement has landed.
TH10: The audit.compensation record MUST carry subject = event and the event_id.
TH11: The scan MUST retry a refused compensating [Record Action].
TH12: The third half MUST treat an owed narration as a repair to record.
TH13: The next run MUST NOT record an owed compensation BEFORE confirming under the section that the retention exists.
TH14: The third half MUST NOT treat an intent past the horizon as a finding.
```
...
WHY: the state step 4 leaves when it fails after step 3 is durable and invisible to every other surface — [Purge Eligible] enumerates retentions, the first half *Purged* retentions, the second half attestations, and an unretained event's attestation is perfectly bound. A scan that read a lost index entry as *no retention exists* would place a second retention over an event that already had one, which Retention Window's Invariant 5 records as new and which falsifies Invariant 2's *exactly one* (TH3). Intent before act lets the trail show the repair was the scan's; placement before compensation keeps the compensation truthful (TH8, TH9). A crash between the two leaves the gap closed but unrecorded, and the intent record is its detector (TH12, TH13) — what can be delayed is the narration, never a false closure and never an unclosed gap. GA2's retention side is this half run from outside.

*Where the compensation lands — the composition is its own event log.*

```text
WL1: The scan MUST record EVERY finding as an audit.reconciliation event through [Record Action].
WL2: The scan MUST record one audit.reconciliation record per finding.
WL3: The scan MUST NOT write a compensating act BEFORE the finding's audit.reconciliation record has landed.
WL4: The scan MUST record EVERY compensating write as an audit.compensation event through [Record Action].
WL5: The scan MUST supply actor_ref = reconciliation_operator and credential = reconciliation_operator_credential at EVERY reconciliation-path [Record Action].
WL6: The composition MUST NOT grow a second store for the reconciliation history.
WL7: The composition MUST NOT add an action for the reconciliation path.
WL8: The composition MUST place a reconciliation-path event under retention_policy like any other event.
```

WHY:
The findings and the compensating writes are audit events — attested, sequenced, retention-governed and sealed like the events they are about — so *what did this system do about the gap it found?* has a first-class answer from the same traversal. One record per finding keeps any findings set inside `payload_cap`. The reserved namespace (PP7, PP8) is the difference between a marker that is evidence of a compensation and one that is anybody's claim. The scan is an ordinary caller of [Record Action] (WL5); `execution-contract.md` has a composition record a multi-step sequence by composing Event Log, never by growing a second store, and this composition *is* an Event Log composition (WL6, WL7).
...
```text
LV1: WHEN store outage EXISTS:
    LV1a: The liveness arms of Invariants 1, 2 and 8 MUST suspend for the outage.
    LV1b: EVERY outstanding finding MUST stay surfaced through the outage.
```
...
Terms › `recorded through [Record Action]`: the quantifier of Invariants 1, 2 and 8 — this composition declares exactly one way in, and an event written around it is not one these invariants cover (NG2).
...
- **Invariant 1 — Attribution coverage (safety + liveness at quiescence).**
  ```text
  IV1: An action of this composition MUST NOT leave an event in the audit log with the event's attestation side unbound and unsurfaced.
  IV2: [Record Action] MUST NOT append BEFORE attesting.
  IV3: EVERY appended event MUST carry a committed attestation_id inside the event's own payload.
  IV4: The scan MUST surface and reconcile EVERY orphan WITHIN compensation_window of the orphan's attested_at.
  IV5: WHEN quiescence EXISTS:
      IV5a: For EVERY event_id recorded through [Record Action], event_to_attestation's entry MUST reference a recorded attestation carrying a readable action_ref and actor_ref.
      IV5b: EVERY attestation in the store MUST fall in EXACTLY ONE OF the binding set, compensated_attestations.
  IV7: WHEN retention_state != Purged:
      IV7a: The attestation's action_ref and actor_ref MUST match the event payload's, byte for byte.
  IV8: WHEN retention_state = Purged:
      IV8a: The attestation the pair names MUST exist with readable surviving fields.
      IV8b: An auditor MUST evaluate IV5a against the attestation store's surviving fields alone.
      IV8c: An auditor MUST NOT read the who / what / when from the destruction record.
  IV9: A new orphan a compensating write leaves MUST count as a new finding with the new orphan's own attested_at.
  IV9a: The scan's next run MUST retry EVERY orphan not yet reconciled.
  IV9b: The composition MUST NOT condition convergence on a compensating write succeeding.
  ```
  *Rests on:* [Record Action] steps 2, 3 and 5; the liveness arm on [Record Action] itself — the compensating record is written through it under `audit.compensation` and the finding under `audit.reconciliation` (WL1–WL5), with the observable closure on `compensated_attestations` pre-checked under the per-`attestation_id` section (SH8, CC4); Actor Identity Invariants 1 (attestation immutability), 2 (action binding), 3 (actor binding) and 9 (attestation durability — why the closure needs a marker at all, since the orphan it forecloses deleting is permanent); Event Log Invariants 1 (append-only) and 2 (event immutability).

  WHY: the reverse partial is reachable and durable, since synchronous rollback is unavailable, so the honest claim is a surfaced transient under compensation, never a quiet inconsistency. *Reconciled* is membership in `compensated_attestations` because nothing about the attestation itself ever changes to say *dealt with*; the marker is not forgeable (PP7, RO2, GA7). A compensation is an ordinary [Record Action] and can itself fail at step 3, leaving a new orphan; the chain terminates the way retries terminate, each orphan bounded from its own creation, no link unsurfaced (IV9). The clause compares differently by retention state because the cascade destroys one side of the comparison and not the other.

- **Invariant 2 — Retention coverage (safety + liveness at quiescence).**
  ```text
  IV10: A successful [Record Action] MUST NOT return an event_id with no retention record.
  IV11: The scan MUST reconcile EVERY unretained event WITHIN compensation_window of the event's recorded_at by placing the missing retention.
  IV12: The reconciliation path MAY place a retention ONLY IF true miss EXISTS for the event_id.
  IV13: WHEN quiescence EXISTS:
      IV13a: For EVERY event_id recorded through [Record Action], event_to_retention's entry MUST reference EXACTLY ONE recorded retention of either retention_state.
  ```
  *Rests on:* [Record Action] steps 3, 4 and 5; the liveness arm on [Record Action] itself (WL1–WL5); the composition's own `event_to_retention` pre-check under the per-act section and below `record_edge` (TH2–TH7, CC3), which supplies the idempotence Retention Window's Invariant 5 nowhere declares; the third half as the arm's detector (TH1); [Purge Event] step 0½ as the arm that keeps the cascade out of the window (PU14); Retention Window Invariants 1 (membership exclusivity), 5 (`record_ref` and `policy_ref` immutability, and the new-retention-on-re-retention-under-a-different-policy rule that makes the pre-check necessary) and 10 (retention store durability); Event Log Invariant 1.
...
- **Invariant 3 — Integrity coverage (modulo unsealed tail).**
  ```text
  IV14: IF sealed_through EXCEEDS the event's sequence_number OR sealed_through = the event's sequence_number THEN EXACTLY ONE seal MUST cover the event.
  IV15: A purged event MUST remain covered by the covering seal.
  ```
  *Rests on:* [Seal Now] (and [Record Action] step 6 under per-event cadence), with [Purge Event] step 0 keeping it true of every purged event by construction; Tamper Evidence Invariants 1 (evidence immutability), 3 (record-set binding) and 9 (seal store durability, why a seal outlives the records it committed to); Event Log Invariants 3 (total order) and 4 (sequence-number monotonicity), without which a contiguous range is not a well-defined cover.
  ...
  WHY: the claim is exact because there is exactly one way a coverage entry gets written — [Seal Now] cuts the slice and advances `sealed_through` past it, so slices are contiguous, disjoint and strictly advancing (SC7, SC8). *At most one* follows from disjointness and *at least one* from the bound; the claim is over seals, full stop, with no second class of seal to quantify around. What a purged event loses is verifiability, not coverage, and [Verify Record] reports that honestly. The claim is unconditional rather than modulo a seal lifetime because seals are retained indefinitely (RP5); Erasure Tombstone composed for seal disposal and Seal Lifecycle composed for re-sealing would each put it modulo their own bookkeeping, and both are named and out of scope. The unsealed tail is a bounded gap the cadence shrinks and `unsealed_tail_mode` names what a verifier makes of it.

- **Invariant 4 — Cascade coordination on purge.**
  ```text
  IV16: A [Purge Event] MUST leave the four stores in the coherent end state: the event covered, the retention in Purged with purged_at, the sequence_number in purged_events with the pair captured, and the destruction delegated with the outcome recorded.
  IV17: The cascade MUST commit the steps in the declared order.
  IV18: The composition MUST NOT claim an atomic set spanning the retention transition and the delegated destruction.
  IV19: An auditor MUST clear the delegated destruction's completion outside the records.
  ```
...
- **Invariant 7 — Verification asymmetry preserved.**
  ```text
  IV26: [Verify Record] MUST require the original record set re-presented.
  IV27: [Verify Record] MUST NOT require the actor's credential beyond the registry's public material.
  IV28: The composition MUST NOT fetch the covering record set internally for verification.
  ```
  *Rests on:* [Verify Record] steps 3 and 5; [Read Record] step 4, which supplies the covering range (RR10); PP22–PP25, which fix both shapes; Tamper Evidence Invariants 3 (record-set binding) and 4; Actor Identity Invariant 6.

  WHY: the asymmetry inherits Tamper Evidence's verification self-containment given the originating records and Actor Identity's verification self-containment, and it surfaces at the caller boundary — which is why [Read Record] does not verify (RR15): a read surface that fetched the payload internally would hide exactly this asymmetry. The asymmetry extends to cadence rather than being narrowed by it: the verifier presents what the covering seal commits to (PP23), and only its extent moves; what the composition owes in exchange is knowing which record set that is, discharged through [Read Record]'s returned range.

- **Invariant 8 — Honest representation of destruction (at quiescence).**
  ```text
  IV29: WHEN quiescence EXISTS:
      IV29a: EVERY event the composition recorded MUST stand in EXACTLY ONE OF retained, lawfully destroyed.
  IV30: A retained event MUST carry a retention record in Retained.
  IV31: A lawfully destroyed event MUST carry a retention record in Purged with purged_at, the sequence_number in the covering seal's purged_events, the pair in the destruction record, and a destroyed outcome in erasure_outcomes.
  IV32: [Verify Record] MUST NOT read purged off the absence of content.
  IV33: The composition MUST reserve not-known for an event_id no constituent has heard of.
  ```
  Terms › `event standing`: `retained` | `lawfully destroyed`.

  *Rests on:* [Purge Event] steps 0, 1, 2 and 4 and [Verify Record] steps 1–2; [Read Record] step 5 and [Verify Record] step 2 for the compensation-window reading (RR12, VR10); the liveness arm — that the compensation-window state is bounded and closes — on [Record Action], through which the scan records its findings and the compensating placement (WL1–WL5); Retention Window Invariants 1 (membership exclusivity), 3 (terminal absorption) and 10 (retention store durability); Tamper Evidence Invariant 1; Invariant 2 above for the window it is conditioned on and Invariant 4 for the coordination that produces the surviving records.

  WHY: *missing without record* does not occur through this composition. The fourth record is why the foreclosed state stays foreclosed, and the mechanism is the scan: a `destruction-failed` delegation leaves *Purged* over readable content, and FH4's predicate re-drives it every cycle — foreclosed because the scan retries, not because failure was assumed away. Every purged event has a covering entry by construction (PU9). Without the quiescence qualifier this invariant would contradict Invariant 2's liveness arm; inside the window the event is verifiable but not yet retention-covered, a bounded finding, not a third steady state. Reading `purged` off the record rather than off the absence of content is what makes the distinction survive every conforming erasure mechanism.
...
**Regulator audit — "show me the complete, verifiable history of action X over the retention horizon."** *Every event referencing action X* is a query by payload field, so the enumeration is a composed Reverse Index *(forthcoming)*, not a passthrough (AW3, AW4); a sequence-number or wall-time window would pass straight through. From there the composition answers per event: `verified` for each retained event, the *Purged* retention record for each destroyed one. Invariants 1, 2, 3 and 8 are the structural answer, from the records, without source code or runbooks.
...
```text
GA1: An auditor MUST obtain, from one [Read Record] call on any event_id, the joined audit record — what, who, integrity and retention.
GA1a: An auditor MUST confirm that not-known is returned only for an event_id no constituent knows.
GA2: An auditor MUST verify all eight composition-level invariants over the record set.
GA2a: An auditor MUST verify Invariants 1 and 2 at quiescence, reading the quiescence condition from compensation_window.
GA2b: An auditor MUST enumerate the log against the attestation store and the retention store.
GA2b1: IF attestation age EXCEEDS audit edge THEN an auditor MUST read an unreconciled orphan as a residual finding.
GA2b2: IF event age EXCEEDS audit edge THEN an auditor MUST read an unreconciled unretained event as a residual finding.
GA2c: IF audit edge EXCEEDS a finding's age THEN an auditor MUST NOT read the finding as a residual finding.
GA2d: IF attestation age EXCEEDS horizon THEN an auditor MUST read the orphan against the scan's beyond-horizon audit.reconciliation findings.
GA2e: An auditor MUST NOT read a purged compensation of a standing orphan as a violation.
GA2f: An auditor MUST run the retention side of the enumeration as the mirror of the third half: the full enumeration, the per-event test of event_to_retention with rebuild-on-miss first, and a true miss past the window read as a finding.
GA2g: An auditor MUST build the binding set and take EVERY attestation in neither enumeration as an orphan.
GA2h: An auditor MUST read an orphan in compensated_attestations as reconciled and the orphan's audit.compensation event as the proof.
GA2i: An auditor MUST read an orphan not in compensated_attestations, past compensation_window, as the residual finding.
GA2j: IF compensation_window NOT EXISTS THEN an instance MUST fail GA2.
GA3: An auditor MUST verify all seven Event Log invariants over the audit log instance.
GA3a: An auditor MUST read a violation of Event Log Invariant 7 as a clock finding.
GA3b: An auditor MUST verify Actor Identity's, Retention Window's and Tamper Evidence's Generation-acceptance bars over the respective instances.
GA4: An auditor MUST bound the forensic window of any detected tampering by the latest verified seal and the first failed seal, with the seal stamps.
GA4a: An auditor MUST walk EVERY entry in the seal store.
GA4b: An auditor MUST step over a seal answering unverifiable(partially-purged-coverage) to the nearest presentable seal.
GA4c: An auditor MUST read the mechanism's chaining posture from the deployment's declaration.
GA5: An auditor MUST confirm, for EVERY purged event, that failed-verification(purged) is backed by the records of GA5a–GA5e.
GA5a: An auditor MUST confirm the retention record in Purged.
GA5b: An auditor MUST confirm the event's own sequence_number in the covering seal's purged_events.
GA5b1: An auditor MUST read a purged event with no covering entry as a conformance failure.
GA5c: An auditor MUST confirm the pair in the destruction record.
GA5d: An auditor MUST confirm the attestation the pair names exists with readable surviving fields.
GA5d1: An auditor MUST read a mechanism that destroyed more than the data field and the proof as non-conforming.
GA5e: An auditor MUST confirm a destroyed outcome recorded for the event in erasure_outcomes.
GA5e1: An auditor MUST NOT read a destruction-failed record as completion.
GA5f: An auditor MUST confirm the live members of a partly-purged seal answer unverifiable(partially-purged-coverage).
GA5f1: An auditor MUST read an instance answering verified for a live member of a partly-purged seal as composing Seal Lifecycle, audited against that pattern's bar.
GA5g: An auditor MUST confirm an event with no retention record inside compensation_window carries retention status unresolved (compensation window).
GA5h: An auditor MUST confirm [Purge Event] over an event with no retention record inside compensation_window returns rejected(retention-unresolved).
GA6: An auditor MUST confirm that verification could not be performed surfaces as unverifiable(reason) and never as failed-verification(reason), for attestation-registry-unavailable, seal-mechanism-verification-unavailable and partially-purged-coverage.
GA7: An auditor MUST discard event_to_retention, event_to_sequence, sealed_through, seal_coverage's ranges, compensated_attestations and reported_beyond_horizon, run the rebuild procedures against the constituent stores, and reproduce EVERY traversal answer.
GA7a: An auditor MUST discard event_to_sequence first.
GA7b: An auditor MUST confirm EVERY member of compensated_attestations came from an event carrying an audit.* action_ref whose payload actor_ref = reconciliation_operator.
GA7c: An auditor MUST regenerate event_to_attestation's entry for EVERY live event from the event's payload.
GA7d: An auditor MUST confirm event_to_attestation's entry for EVERY purged event is present in the destruction record.
GA7e: An auditor MUST read a purged event with no destruction-record pair as a conformance failure.
GA7f: An auditor MUST confirm the scan's binding set and event_to_attestation's two halves agree.
GA7g: An auditor MUST NOT expect a rebuild of an extraction-pending fact.
GA8: An auditor MUST identify the composing patterns active in the deployment and the patterns' configuration.
GA8a: An auditor MUST NOT run GA1–GA7 BEFORE identifying Reverse Index, Legal Hold, Erasure Tombstone composed for seal disposal, and Seal Lifecycle.
GC1: An implementation derived from this composition MUST produce records and a runtime surface that clear GA1–GA8.
GC2: An implementation derived from this composition MUST make EC1–EC6 askable, naming the external evidence each needs.
```
...
Terms › `residual finding`: an unreconciled finding older than the audit edge — the conformance failure GA2 reports.
...
WHY:
GA2 reads quiescence off a declared number, not the spec's confidence; its retention side is the third half run from outside (GA2f), and its orphan side is audited against a marker because Actor Identity Invariant 9 makes *no orphans remain* unreachable — what a conforming instance reaches is *every orphan compensated* (GA2g–GA2i). Event Log carries no acceptance bar, so its seven invariants are verified directly (GA3). GA4 clears as a window only under a chained mechanism, which opacity keeps out of the records (GA4c). GA5 reads records, not the absence of content, and mere presence of an outcome record would clear the state Invariant 8 forbids (GA5e1). GA6's third reason is the sharpest, because the composition's own cascade created the condition. GA7b is what makes `compensated_attestations` unforgeable; the three extraction-pending facts are verified by GA5 instead (GA7g). GA8's full list: Legal Hold, Defensible Retention, and *(forthcoming)* Trusted Timestamping, Storage Tier, Compromise Disclosure, Erasure Coordination, Policy Reconciliation, Mechanism Registry, Reverse Index, Legacy Import, Failed-Attempt Log, Schema Evolution, Erasure Tombstone, Seal Lifecycle, and the declared erasure mechanism; the four GA8a names change what the other checks mean.

### Externally-clearable checks

```text
EC1: An auditor MUST clear, outside the records, that the erasure mechanism was authorized to destroy, is shredding-class, and completed each destruction.
EC2: An auditor MUST clear, outside the records, that the actor registry retained the registry's historical public material across key rotation.
EC3: An auditor MUST clear, outside the records, that the seal mechanism is cryptographically sound for the audit horizon.
EC4: An auditor MUST clear, outside the records, that the mechanism's seal-time rendering agrees with the verify-time presentation.
EC5: An auditor MUST clear, outside the records, that the clock was monotonic and unmanipulated over the audit horizon.
EC6: An auditor MUST clear, outside the records, that the configured retention_policy was the correct one for the record class.
```

WHY:
The records show a changed stored field against the immutability invariants and cannot show that the key really went away — the direct cost of the delegation, part of which migrates to the traversal list once Erasure Tombstone lands (EC1). A registry that drops superseded material fails old attestations for a reason that has nothing to do with the trail (EC2). A rendering mismatch surfaces as `failed-verification(seal-proof-invalid)`, indistinguishable from tampering — the worst ambiguity for the one answer this composition exists to give (EC4). Every stamp is only as truthful as the injected clock (EC5). Code generated from this composition must clear the eight traversal checks and make the six external questions askable, naming the evidence each needs.
...
- **Non-goals.**
  ```text
  NG1: The composition MUST specify one instance.
  NG2: The composition MUST NOT provide a path for a pre-attestation legacy event.
  NG3: A Legacy Import pattern MUST NOT write into the audit log around [Record Action].
  NG4: A Legacy Import pattern MUST take EXACTLY ONE OF attesting each imported record under a declared import identity through [Record Action], a separate unattested store the auditor reads as explicitly unattributed history.
  NG5: The composition MUST NOT invalidate an attestation retroactively.
  NG6: The composition MUST NOT adjudicate an erasure request against a retention obligation.
  NG7: The composition MUST NOT declare a seal-store retention owner.
  NG8: The composition MUST NOT rotate a seal onto a new mechanism.
  NG9: seal_mechanism MUST govern seals cut from the time of setting on.
  NG10: The composition MUST NOT treat a storage tier differently at [Verify Record].
  ```
  WHY: multi-instance configuration and federation are the deployment layer's — an Audit Federation pattern *(forthcoming)* composes naturally (NG1). An import identity binds the importer, not the original actor; whether the attribution gap is acceptable is a legal question about the imported body (NG2–NG4). Attestations made during a compromise window verify but should be reinterpreted by new records — Compromise Disclosure *(forthcoming)* (NG5). A GDPR Article 17 request colliding with a retention obligation is Erasure Coordination's *(forthcoming)* decision, with counsel in the loop (NG6). A coarse cadence has a second cost that arrives years later: the more live seal-mates each purge strands. Seals already written stay under the mechanism that produced them (NG8, NG9). Storage Tier *(forthcoming)* owns the active-to-cold transition (NG10).
- **Legal hold suspension of purge.** Where the composed hold lands is [Purge Event], not [Purge Eligible] (PU5, PE6); [Defensible Retention](./defensible-retention.md) already wires the gate over a business record set. A deployment wanting a hold-filtered worklist asks the composed Legal Hold pattern.
- **Concurrency.** Four serialization obligations, all implementation-owned.
  ```text
  CC1: The implementation MUST serialize the read-seal-advance sequence per instance across [Seal Now], the cadence-fired seal and [Purge Event] step 0.
  CC2: The implementation MUST serialize the cascade per event_id.
  CC2a: The implementation MUST protect the destruction-record write against a lost membership.
  CC2b: A per-evidence_id serialization MAY discharge CC2a.
  CC2c: An atomic set-add MAY discharge CC2a.
  NOTE: watch cardinality — an inclusive *either discharges it* has no form; written as one obligation and two MAY rules (CC2a–CC2c). The same pressure at SH11 (one writer) and WL2 (one record per finding).
  CC3: [Record Action] steps 3–5 MUST run under the per-act section keyed by the attestation_id step 2 returned.
  CC3a: [Record Action] steps 1–2 MUST NOT require composition-level serialization.
  CC3b: [Record Action] step 6 MUST take the per-instance sealing lock of CC1.
  CC4: The scan MUST serialize an orphan's compensation per attestation_id.
  CC5: [Read Record] and [Purge Eligible] MUST NOT take a serialization lock.
  ```
  WHY: two sealings reading the same `sealed_through` would produce two `evidence_id`s over one range, violating Invariant 3, and Tamper Evidence will not stop them — one seal per record set per cadence is the composing pattern's job (CC1). The cascade and its second step contend on different resources: the cascade on `event_id` — the per-act section for a cascade, its lease `purge_completion_bound` long — while the step-2 write contends on the covering seal's entry, shared by every member of the range, where a read-modify-write under the race silently loses a membership no rebuild brings back; the per-`event_id` lock alone is the natural mistake (CC2, CC2a). The record action's section exists from the act's first write, which a per-`event_id` key could not; step 6 is the read-seal-advance sequence CC1 governs (CC3). An orphan has no `event_id` to lock; unserialized, a restart scan and a cadence scan both read *absent* and both record — duplicates are not false, but they make *once, or looping?* a question answered by reading timestamps (CC4).
- **Durability across crashes.** The obligation follows the classification, not the element list.
  ```text
  DU1: The composition MUST treat a crash that records the event and loses a derived-index entry as a rebuild trigger.
  DU2: The reconciliation path MUST NOT read a lost event_to_retention entry as absent.
  DU3: An id-addressed action MUST NOT treat an event_to_sequence miss as not-known.
  DU4: A short rebuild of a closed-state marker MUST NOT produce a false record.
  DU4a: A short rebuild of a closed-state marker MAY produce a duplicate record.
  DU5: The deployment MUST persist purged_events membership, the pair and erasure_outcomes with the cascade's write.
  DU6: The implementation MUST own the transactional boundary for the truth-bearing writes.
  ```
  WHY: `event_to_retention`'s rebuild-on-miss is load-bearing because Invariant 2's placement pre-checks it (DU2); `event_to_sequence`'s is the hottest, because every id-addressed action resolves through it (DU3). `compensated_attestations` is safe in the direction that matters — nothing enters the set except a compensation that was recorded (DU4). The membership and the pair are unreproducible because the cascade destroys the payload the rebuild would read; `erasure_outcomes` because re-driving over destroyed content need not answer `destroyed` again (DU5). `event_to_attestation` is a discardable index for a live event and durable truth for a purged one, and an implementation that treats the map uniformly gets one half wrong. Until Erasure Tombstone lands, the deployment owns persisting all three.
- **Cross-store consistency under failure.**
  ```text
  XS1: The composition MUST require ordered writes plus compensation for a sequence of un-withdrawable writes.
  XS2: The composition MUST NOT require a rollback of a committed constituent write.
  XS3: The implementation MUST own how the implementation's own process survives the gap between two commits.
  ```
  WHY: if `EventLog.append` succeeds and `place_under_retention` fails, the composition is in a state Invariant 2's safety arm forbids at quiescence, and append-only forecloses withdrawing the event; the attestation and the append are each un-withdrawable, so *all succeed or none* is not a state the constituents can offer (§*Durability boundaries*). The failure is surfaced as `rejected(recording-failure(step))`, recorded as a compliance finding, and reconciled within `compensation_window`; the finding and the compensating write are audit events through [Record Action] (WL1–WL8), never an operational log. The scan's three predicates are all stated (FH, SH, TH); the cascade's own half-completed state needs the first half because it is invisible to [Purge Eligible].
- **Verification of the unsealed tail.**
  ```text
  TV1: The deployment MUST monitor tail depth and age against seal_cadence.
  TV2: The deployment MUST read a [Mechanism Failure] with a preconditions reason as a standing misconfiguration.
  TV3: The deployment MUST read a [Seal Now] invalid-request as a standing defect.
  ```
...
- **Failed attribution attempts.**
  ```text
  FA1: The composition MUST NOT record a [Record Action] rejected at step 1.
  FA1a: The composition MUST NOT record a [Record Action] rejected at step 2.
  ```
...
- **Partial attestation on step failure.** The state, the surfacing and the closure are owned where they happen: the invocation surfaces the orphan in `rejected(recording-failure(step-3))` and writes no compensation (RA43, RA47); the scan is the one writer of the compensating record (SH11), finds the orphan by the binding set (SH1, SH2), examines nothing younger than `record_edge` (SH3), and pre-checks `compensated_attestations` (SH8); reconciled has the observable form the term `reconciled` names; the compensating record is an audit event (WL4). High-assurance deployments treat any orphan not in the set, past the window, as a gap in the audit surface and alert.
- **Clock source for cadence and purge.**
  ```text
  CK1: The composition MUST NOT read now other than for the seal_cadence timer and the scan's once-per-run reading.
  CK2: The composition MUST NOT run the transition BEFORE reading now at the composition's I/O seam.
  CK3: An invocation in flight MUST NOT read a clock.
  CK4: The deployment MUST supply a monotonically non-decreasing clock.
  ```
  Terms › `invocation`: a [Record Action] or a [Purge Event] between the invocation's first write and the invocation's return.

  WHY: the orchestration transition is a pure function of injected `now` (`execution-contract.md` §Logic Confinement). An invocation's terminus at the completion bound is the expiry of a lease the host times, and on a death-detected host it is *proceed as landed*, which needs no reading. Purge eligibility is not evaluated here (PE1–PE4). Clock skew across nodes can cause non-deterministic eligibility and inconsistent cadence firing; the deployer configures the source — system clock, GPS-disciplined, NTP-synchronized cluster — and owns monotonicity, or composes a Trusted Timestamping pattern *(forthcoming)* whose anchored time serves as the authoritative source.

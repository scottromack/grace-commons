...
Composes 15 through 24 are the two directions each authorization surface is used in, and the asymmetry is the composition's whole shape: every surface is **read** to build the plan and **written** to close it, and no constituent offers a bulk close. Permissions names per-grant enumerate-then-revoke as the composing system's responsibility and Session supports the same over its principal reference-queryable store; this composition is that composing system. Composes 21 and Composes 22 are the other half of the honesty: it revokes and never issues, which is exactly why a grant created after the snapshot is outside the guarantee and the issuance gate is a deployment obligation rather than a claim made here.
...
Composition state 46: The composition MUST derive an enumerated active set instant the snapshot.
...
Term suspension-state index: actor_suspension_state — the composition's index from an actor reference to the actor's lifecycle state and the record that put the actor there; the surface the issuance gate reads and [Suspension Report] answers from.
...
Capability requirement 29: The host MUST supply a critical section PER actor reference.
...
Capability requirement 33: The suspension completion bound MUST NOT EXCEED a leased critical section's lease.
...
Term unified actor namespace: unified_actor_namespace — the deployment's declaration that the actor reference this composition suspends is the same identity value under which the actor holds grants as subject reference, sessions as principal reference and credentials as Credential's own principal reference.

Term critical section: per_actor_serialization — the host-supplied mutual exclusion keyed by actor reference, taken at the state gate and held through the index write, taken by a resume, and taken by the sweep for every actor the sweep examines.
...
WHY:
Capability requirement 12 through 15 are the enumeration's declaring source, and they are the composition's largest audit gap stated as an obligation rather than a claim. This composition enumerates grants by subject reference, sessions by principal reference and credentials by Credential's own principal reference — three namespaces — and the enumeration is complete only where all three coincide with the actor reference. A deployment satisfies that through [Authenticated Actor](./authenticated-actor.md)'s binding or by convention; whether it actually did is External check 1, because verifying that two opaque namespaces coincide is not a records-alone question at this layer. A divergent namespace under-enumerates *silently*, which is why the knob is a declaration the deployment makes rather than a default the composition assumes.
...
Primitive policy 2: The boundary predicate MUST refuse a blank actor reference.
...
Term opaque input: actor reference | suspended by ref | reinstating actor ref | credential | grant id | session token | credential id.

Term operator reference: suspended by ref | reinstating actor ref.
...
Identity 12: The composition MUST NOT pair an outcome to a cascade by the actor reference.
...
Identity 16: An intent MUST carry the injected now as intent instant.
...
Term intent instant: the instant an intent records (Identity 16).
...
Term suspension instant: the instant a suspended outcome records (Action wiring 48).

Term reinstatement instant: the instant a reinstated outcome records (Action wiring 84).
...
Term suspension record: state, an optional intent event id, an optional open_members, an optional suspension instant, an optional suspended by ref, an optional reason, an optional revoked_grants, an optional revoked_sessions, an optional revoked_credentials, an optional unresolved_members, an optional enumeration_availability and an optional suspension event id — what suspension_report answers.
...
```
Action wiring 1: The composition MUST take the actor's critical section at [Suspend Actor] ONLY AFTER the boundary predicate.
Action wiring 2: The composition MUST read the suspension-state index at [Suspend Actor] ONLY AFTER the actor's critical section.
Action wiring 3: The composition MUST hold the actor's critical section at [Suspend Actor] through the index write.
...
Action wiring 22: A cascade MUST hold the actor's critical section at every revocation.
Action wiring 23: IF the actor's critical section stands lost THEN the invocation MUST retake the actor's critical section.
Action wiring 24: IF the actor's critical section stands unavailable THEN [Suspend Actor] MUST answer recording-failure carrying outcome.
...
Action wiring 48: A suspended outcome MUST carry the invocation id, the intent event id, the suspended actor, the operator, the revoked set, the unresolved members, the reason AND the injected now as suspension instant.
...
Action wiring 73: IF the actor's lifecycle state EQUALS suspended THEN [Suspension Report] MUST answer the state, the suspension instant, the operator, the reason, the suspension event id AND the revoked set.
...
Action wiring 77: IF the outcome stands aged out THEN [Suspension Report] MUST answer the state, the suspension instant, the operator AND the suspension event id from the suspension-state index.
...
Action wiring 84: A reinstated outcome MUST carry the invocation id, the reinstated actor, the reason AND the injected now as reinstatement instant.
...
Reconciliation 10: The sweep MUST take the actor's critical section PER candidate.
Reconciliation 11: The sweep MUST skip a candidate whose critical section stands unavailable.
Reconciliation 12: The sweep MUST hold the mark below a skipped candidate's intent.
Reconciliation 13: The sweep MUST take the pre-check under the actor's critical section.
Reconciliation 14: The sweep MUST NOT take the pre-check BEFORE the actor's critical section.
...
Term young intent: an intent whose intent instant stands within the suspension completion bound of the injected now.
...
Reconciliation 10 through 18 are the one-writer rule. The pre-check is re-read *under* the critical section and never before it — Reconciliation 14 — because a pre-check taken outside the critical section is a read whose answer another writer may invalidate before the append, which is exactly the race that produces two sealed outcomes for one act. A candidate whose critical section the sweep cannot take is skipped rather than forced, and the hold is bounded: the lease is the completion bound, and an invocation past it has yielded.
...
2. **State gate.** The critical section for `emp_4821` is taken, the index entry is absent, and the tail read above the mark finds no open intent → active → a fresh cascade.
...
6. **Outcome.** The pre-check under the critical section finds no outcome for this invocation, so `actor.suspended` lands carrying the invocation's id, the intent's id, the operator, the full revoked set, an empty unresolved set and the reason; the suspended entry is written after it.
...
External check 7: The deployment MUST establish that the host's critical section excludes across every node (Capability requirement 29).
...
**Check 6.5 and Check 6.6 check the inequality and not a fragment of it.** A deployment whose cadence fits the window while the bound does not breaches on every orphan with the fragment passing, which is why the terms are confirmed together — the section titled *Liveness is arithmetic* in `pressure-testing.md`.
...
**Non-goal 18 is why the completeness guarantees live at this layer at all.** Permissions offers no bulk revoke — its own mass-revocation edge case names per-grant enumerate-then-revoke as the composing system's job — and Session's bulk revocation is the same shape over its queryable store. This composition is that composing system, performing both enumerations and every per-member revocation under one critical section, recording the plan before and the outcome after. Neither atom could have carried Invariant 1 or Invariant 2, because neither can see the other's surface.
...
Deleted: Clock semantics 2. Identity 16, Action wiring 48 and Action wiring 84 own it: intent instant, suspension instant and reinstatement instant are every timestamp this composition stamps, and each takes the injected now.
...
Deleted: Clock semantics 13. Capability requirement 1 and Execution Contract Logic confinement 7 own it: the seam supplies now, and the reading's honesty is the deployment's.
Deleted: Clock semantics 14. Non-goal 19 owns it.
```

WHY:
Clock semantics 6 and Clock semantics 9 are one rule about where correctness comes from. The cascade's correctness rests on the critical section and on the log's insertion order, never on comparing clocks: no write on this page is deciding actor a cross-seam comparison, which is the section titled *A stamp from another seam never decides a write alone* in `pressure-testing.md` satisfied by construction rather than by care. Clock semantics 12 is the one comparison the composition does make, and it compares two readings of *one* clock — the intent's stamp against the sweep's own — which is why the sweep's lower edge is sound without a skew term.
...
```
Concurrency 1: A deployment MUST serialize a state-changing call over one actor reference.
...
The composition's load-bearing emergent action: under the actor's critical section it snapshots every stored-active grant and session the actor holds — and, where the credential arm stands composed, every effective-active credential across every type — sizes the plan against the cap, records the plan under the operator's verified credential, revokes each member, seals the complete revoked set into one attributed event, and moves the actor from active through suspending to suspended. Against an already-suspended actor it is a no-op; against a suspending one it **resumes** the open cascade under its own attested resume record. Neither constituent carries this multi-surface cascade, and neither could: neither can see the other's surface.
...
- **SOX (Sarbanes-Oxley Act) section 404 (Internal Control over Financial Reporting)** — segregation of duties and access removal on role change are section 404 controls; the enumerated, attributed, tamper-evident outcome is the evidence that a departing or reassigned actor's access to financial systems was removed promptly and completely.
- **HIPAA (the US Health Insurance Portability and Accountability Act) section 164.308(a)(3)(ii)(C) (Termination Procedures)** — the administrative safeguard requiring procedures for terminating access to electronic protected health information when a workforce member's employment ends. This composition is the structural realization.
...
- **ISO/IEC 27001 clause A.9.2.6 (Removal or Adjustment of Access Rights)** — the International Organization for Standardization / International Electrotechnical Commission control requiring access rights be removed on termination or adjusted on change. The enumerate-and-revoke-each-surface cascade is the operational control; the report and the sealed outcome are the audit evidence.

Actor Suspension inherits the broader standards compliance of its constituents:

- Through **Permissions**: NIST SP 800-53 AC-2/AC-3/AC-6, NIST SP 800-207 (Zero Trust), ISO/IEC 27001 clause A.9, HIPAA section 164.312(a)(1), SOX section 404, PCI DSS Requirement 7 — the grant-revocation surface the cascade drives.
- Through **Session**: NIST SP 800-63B section 7 (Session Management), OWASP ASVS (the Open Worldwide Application Security Project's Application Security Verification Standard) V3, HIPAA section 164.312(a)(2)(iii) — the session-revocation surface.
- Through the **Audit Trail substrate**, and transitively Event Log, Actor Identity, Tamper Evidence and Retention Window: SOX section 404 and section 802, HIPAA section 164.312(b) and section 164.530(j), PCI DSS Requirement 10, 21 CFR (Title 21 of the US Code of Federal Regulations) Part 11, SEC (the US Securities and Exchange Commission) Rule 17a-4, ISO/IEC 27001 clause A.12.4, GDPR (the EU General Data Protection Regulation) Articles 30 and 32 — the attributed, retained, tamper-evident record the suspension lands on.

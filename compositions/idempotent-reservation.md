...
Capability requirement 6: The host MUST supply one section per token across EVERY instance sharing the token results map.
Capability requirement 7: The host MUST release a section on the holder's return.
Capability requirement 8: The host MUST release a section on the holder's death.
...
Action wiring 4: The composition MUST call Duplicate Prevention's check under the section.
Action wiring 5: The composition MUST release a token's section on EVERY return.
Action wiring 6: The composition MUST NOT write on the strength of a pre-check the composition read under a lapsed section.
...
The record-versus-attempt line falls here as it does elsewhere: what *stands* — an entry, its digest, its result, its instants — clears from the map; what was *attempted* leaves nothing. A concurrent pair that both passed the pre-check writes one entry, not two, so the absence of the section shows up in the commitment store as a double rather than in the map as a defect, which is why External check 1 exists.
...
Non-goal 10: A deployment needing an authorized call MUST compose permissions(../atoms/permissions.md).
Non-goal 11: The composition MUST NOT record an act.
Non-goal 12: A deployment needing an act recorded MUST compose audit trail(./audit-trail.md).

...
- **Invariant 1 — chain completeness.** A chain's step count equals its approver set's count (the spec's Invariant 1.1). Enforced by writing all N rows inside the same transaction as the `chain` row in `initiate_chain`; nothing in SQL prevents a short chain from existing if you skip the rest of the transaction, and the `chain` table stores no approver set — the declared set is the one the `chain_initiated` event carries. Verifier: `invariants.test.ts` `invariant 1a` compares each chain's step count with the `approver_set` in its `chain_initiated` event.
